...
<rubric>
  ...
  <dimension name="structure">
    <description>Does the formatting match the prompt's complexity? Structure should be proportional — not absent, not excessive.</description>
    <score value="1-2">Wall of text. No formatting, no separation of concerns.</score>
    <score value="3-4">Formatting doesn't match complexity. Either a dense multi-requirement prompt with no visual separation, or a simple prompt with excessive headers.</score>
    <score value="5-6">Basic structure present but mismatched to complexity. A 5+ requirement prompt in flat prose, or a 2-constraint prompt with full headers.</score>
    <score value="7-8">Structure matches complexity. 1-2 constraints → clean paragraph. 3-5 constraints → paragraph + bullet list. 6+ → headed sections. Each format is equally valid for its complexity level.</score>
    <score value="9-10">Structure perfectly proportional. Constraints grouped by concern, no mixing, format chosen for the exact complexity level. Can be scanned to extract requirements without re-reading.</score>
    ...
    <calibration-example score="7">A single paragraph with 3 constraints separated by semicolons or em dashes — each clause distinct and scannable. No headers needed because the complexity is low.</calibration-example>
    <calibration-example score="7">An opening paragraph followed by a flat bullet list of 4 constraints. No headers because there's only one concern grouping.</calibration-example>
    <calibration-example score="8">## Task paragraph\n\n## Constraints\n- bullet list\n\n## Error Handling\n- bullet list\n\nUsed because 6+ constraints span multiple independent concerns.</calibration-example>
    ...
    <calibration-note>A dense single-paragraph prompt with 3 constraints can score 7 on structure — the same as a bulleted prompt with 4 constraints or a headed prompt with 8 constraints. Score the FORMAT MATCH, not the format itself. A flat paragraph is correct structure for low complexity; headers are correct structure for high complexity. Penalize mismatches in either direction: no structure for complex prompts (too flat) OR excessive structure for simple prompts (over-formatted).</calibration-note>
    <score-distinction value="7-vs-8">A 7 uses appropriate formatting for its complexity but has minor grouping issues (related constraints split across sections, or unrelated constraints merged). An 8 groups by concern with no mixing.</score-distinction>
    <score-distinction value="8-vs-9">An 8 has good grouping. A 9 additionally makes each section self-contained — reading any section in isolation gives complete context for that concern.</score-distinction>
    <score-distinction value="9-vs-10">A 9 has excellent grouping. A 10 additionally includes examples delineated from instructions, and uses formatting that can be parsed programmatically.</score-distinction>
  ...
  <dimension name="faithfulness">
    ...
    <calibration-note>Faithfulness scores the RELATIONSHIP between the prompt and the user's intent, not the prompt's absolute quality. A raw user prompt that says "add a background task to detect stale files" has faithfulness 5-6 — it faithfully expresses intent but doesn't serve it well (key constraints missing, failure modes unaddressed). An optimized version that adds error handling, concurrency guards, and file-state classification has faithfulness 7-8 — every addition directly serves the user's goal of building a reliable background task. The additions aren't scope creep; they're intent SERVICE. Score faithfulness based on whether additions SERVE the original goal, not whether the user explicitly requested them.</calibration-note>
    <anti-pattern name="originality-bias">Do NOT give the original prompt higher faithfulness than the optimized version simply because the original is "closer to what the user wrote." The user wrote a starting point, not a specification. Additions that make the user's goal more achievable are faithful enhancements. Only penalize additions that change WHAT the user is trying to accomplish.</anti-pattern>
  </dimension>
  ...
  <dimension name="conciseness">
    ...
    <calibration-note>SHORT IS NOT CONCISE. A vague one-sentence prompt ("add a background task that does X") scores low on conciseness because it is SPARSE — it communicates almost nothing per word. A 700-char prompt that adds error handling, concurrency guards, scheduling, and file-state classification — with each clause load-bearing — scores HIGHER on conciseness because it has greater information density. The question is not "how short?" but "can you remove a sentence without losing something the executor needs?" If every sentence earns its place, the prompt is concise regardless of absolute length.</calibration-note>
    <anti-pattern name="brevity-bias">Do NOT give a short vague prompt a higher conciseness score than a longer precise prompt. "Build a REST API" (5 words) scores 3-4 on conciseness — it's brief but informationally empty. "Build a FastAPI REST API with CRUD endpoints, Pydantic validation, and JSON error envelope" (15 words) scores 7-8 — it's 3x longer but 10x more information-dense. Length without filler is density, not verbosity.</anti-pattern>
  </dimension>
</rubric>
