...
### Changed
- **Pattern Graph 3D — depth + atmospheric / cinematic / tactile canon (F1–F18)** — establishes the 3D Pattern Graph as a brand-canonical medium distinct from the 2D workbench UI, codified in `.claude/skills/brand-guidelines/references/3d-visualization.md` (canon F1–F18 with audit checklist) and aligned with the AGENTS.md "Applies to Web UI only" amendment. The full visual + interaction system from the prior `feature/optimize-pattern-graph` work is now in main alongside complementary improvements (velocity-floor snap on spring physics, zero-length focus guard, MeshStandardMaterial with data-driven emissive intensity, three-light setup with PCFShadowMap), structured as the canonical state — not an exception to 2D rules. **Atmospheric layer** (canon F2/F3/F5/F8/F10): Neural Dust galaxy backdrop (3000-particle `Points` cloud, color `0x88ccff`, additive blending, slow XY drift via `_removeDustAnim`), organic breathing oscillation on every cluster mesh + sibling group children + readiness ring + template indicator ring (per-frame `_breathingAnim`, `±2%` baseline scale via `sin(_breathingTime * 1.5) * 0.02 + 1.0`, hover amplifies to `±12%`; uses module-level `_scratchVec3a` + `Z_AXIS` so per-frame allocation budget stays at zero), glowing energy cores at domain anchor vertex points (`PointsMaterial` with cached 64×64 `__semTopGlowTexture` radial-gradient `CanvasTexture`, additive blending, `size: 0.35`; JSDOM `if (!ctx) → undefined` fallback), hierarchical edge animation pulses (`_removeEdgeAnim` updates per-edge `uTime` uniform ~60Hz across all `_edgeUniforms`), template ring hover behavior (rotation spin + opacity oscillation `0.20–0.50`). Template ring segment count `32 → 64` for smoother visual; readiness ring gains `side: THREE.DoubleSide`. **Synaptic data tendons** (canon F5 shader): `EDGE_DEPTH_FRAGMENT` rewritten with two-sine interference (`flow = sin(vDepth * 0.4 - uTime * 3.0)` + `pulse = sin(uTime * 2.0 + vDepth * 0.1)`); HDR `glowingColor = uColor * (1.0 + organicEnergy * 0.8)` boost feeds the bloom pass dynamically as data flows. **Liquid plasma beams** (canon F7 shader): `BEAM_FRAGMENT_SHADER` rewrite — overlapping fluid sine waves replace geometric step packets; volumetric fresnel core; HDR boost. `RIPPLE_VERTEX_SHADER` smooth spherical shockwave replaces digital jitter. `PlasmaBeam.ts` smooth cylinders (`RADIAL_SEGMENTS 5→12`, `TUBULAR_SEGMENTS 16→24`) + gravity-correct catenary droop (`control.y -= sag`). **Interaction layer** (canon F14–F18): one-shot `_hasAutoFocused` guard on the bird's-eye-view auto-focus block (subsequent `rebuildScene` calls preserve user's camera), `applyHighlight(focusedNodeId)` re-applied at the end of every `rebuildScene` (cyan selection survives async `stateFilter` mutations), `handleNodeClick` simplification — `clustersStore.selectCluster(nodeId)` only, the `$effect` watching `selectedClusterId` drives `focusOn` + `applyHighlight` + tactile feedback (no duplicate work, no race with `getClusterDetail`), tactile feedback on external selection (`clusterPhysics.onBeamImpact` + `beamPool.acquire` from FPS-weapon NDC origin with 800ms sustain), deselect resets camera to origin at distance 80, adaptive `focusOn(target, distance?)` preserves current zoom when `distance` omitted (clamped `[5, 25]`), `_checkLod()` fired per-frame inside the focus animate loop because `OrbitControls.update()` doesn't fire `change` events without damping residual when position is mutated programmatically. **Cinematic post-processing** (canon F13): `EffectComposer` pipeline with `RenderPass` + `UnrealBloomPass(strength=1.5, radius=0.4, threshold=0.85)` + `FilmPass(intensity=0.35, color)`. Render loop switched from `renderer.render()` to `composer.render()`; `composer.setSize` on resize; `composer.dispose()` + every pass's `dispose()` walked in cleanup. **Materials + lighting** (canon F1/F11/F12): cluster + domain anchor fills use `MeshStandardMaterial` with `roughness=0.6`, `metalness=0.0`, `emissive` = domain hex, `emissiveIntensity` data-driven (cluster: `0.6 + 0.8 * clamp((memberCount-1) / 49, 0, 1)`; domain anchor: `0.4` recessed). `castShadow=true` + `receiveShadow=true` on cluster fills (self-shadowing under directional light). Three lights: `AmbientLight(0xffffff, 0.3)`, `DirectionalLight(0xffffff, 0.7)` at `(5, 10, 5)` with `castShadow=true` + `shadow.mapSize.set(1024, 1024)`, `HemisphereLight(0x1a1a2e, 0x06060c, 0.2)`. Shadow map: `THREE.PCFShadowMap` (canonical replacement for the deprecated `PCFSoftShadowMap`). Readiness ring stays `MeshBasicMaterial` per the F4 banner-overlay carve-out. **Spring physics** (canon F9): `ClusterPhysics` semi-implicit Euler with `k=120`, `d=12`, `dt clamp=0.1s`, `velocityFloor=1e-4` snap-to-target — prevents infinite micro-oscillation at rest and makes equality assertions possible. **Adaptive focus** (canon F14): `focus-math.ts` pure helper with zero-length direction guard (NaN cascade prevention in camera-on-target case) + adaptive distance. **Brand alignment** (the over-restrictive interpretation that briefly walked back the atmospherics + tactile layer was rolled back this release; the spec that drove that interpretation is gitignored per project convention and is no longer canonical). Brand-doc coherence audit cleanup: `color-mappings.md` "Taxonomy Visualization (3D Topology)" no longer prescribes 2D-style hover/selection rules — defers to canon F1–F18; cluster colors now correctly described as inherited from parent domain hex (the OKLab-from-UMAP mechanism is the cold-path domain-color discovery only). `SKILL.md` Anti-Patterns table + Canon Terminology table + "Word usage in 3D code" subsection scoped to 2D-UI directories only — `glow`/`halo`/`bloom`/`radiance` and identifiers like `_haloPool` / `glowingColor` / `_breathingAnim` are canonical 3D vocabulary. `references/{component-patterns,layout-and-accessibility}.md` gain top-of-file scope headers. Zero-Effects Directive carves out `backdrop-filter: blur()` for glass-morphism surfaces. **Test gates align with canon**: `brand-compliance.test.ts` retains non-canon post-processing import bans (`GlitchPass`/`DotScreenPass`/`RGBShiftPass`/`GodRaysPass`) + positive regression for `EffectComposer`/`RenderPass`/`UnrealBloomPass`/`FilmPass` import reachability — no banned-word regex against 3D scope. `cleanup-contract.test.ts` requires all 8 canon cancellers (`removeBeamUpdate`, `_removeRingLodUpdate`, `_removeFormationAnim`, `_removeDomainRotation`, `_removeReadinessBillboard`, `_removeEdgeAnim`, `_removeDustAnim`, `_breathingAnim`) PRESENT in cleanup body; `__semTopGlowTexture` is the sole authorized production `globalThis` assignment (canon F2). Test mocks extended: `Points.rotation`, `CanvasTexture`, `EffectComposer`/`RenderPass`/`UnrealBloomPass`/`FilmPass`/`Vector2`. **Audit-fix follow-ups** (independent canon-compliance + code-quality reviewers): canon-compliance verdict was CANON-COMPLIANT 18/18; code-quality reviewer surfaced 1 CRITICAL + 8 MAJOR closed in the same release: `__semTopGlowTexture` CanvasTexture is now disposed + nulled in cleanup with the new `_glowTextureBuilt` sentinel preventing re-init on every `rebuildScene`; `WebGLRenderer.forceContextLoss()` added after `renderer.dispose()` per canon F12 to release the GL context on rapid mount/unmount cycles; `cleanup-contract.test.ts` strengthened with explicit assertions on the dispose call + sentinel reset + `__semTopGlowTexture = undefined` on unmount; `GodRaysPass` reconciled — moved to "No" tier in canon F13 alongside `GlitchPass`/`DotScreenPass`/`RGBShiftPass` (the prior "Avoid with conditional permission" never had a real use case); per-event allocations in the `selectedClusterId` `$effect` swapped to `_scratchVec3a` borrowing (focusOn clones internally so safe; the BeamConfig.color stays a fresh Color since it's stored on the acquired beam); `_breathingAnim` sibling-children loop converted from `.forEach(child => …)` to `for...of` to drop ~3000 closures/sec at 50 clusters × 60fps; `_dustPoints` lifecycle comment corrected (component-scoped, rebuilt on remount). **Validation**: 1788/1788 frontend tests pass; 0 svelte-check errors / 0 warnings (1080 files including the post-processing module type defs). The brand reference's "Audit Checklist" is the source-of-truth for what passes 3D brand review; an implementation matching every checked line is canonical.

## v0.4.19 — 2026-05-09

### Added
- **Brand-driven motion personality across the workbench** — restored the brand "spring-loaded motion … precision hardware" cadence across modals, drawers, tabs, list operations, and toasts via shared primitives. Pre-fix only the cluster navigator had any motion, and its scroll-into-view fired during the parent slide animation so the selected row drifted out of view before layout settled — every other surface used static, instantaneous A→Z transitions. **Phase 1 — shared substrate**: `frontend/src/lib/utils/transitions.ts` gains a Newton-Raphson `easeExit` solver for `cubic-bezier(0.4, 0, 1, 1)` (Svelte's stock `cubicOut` drifts visibly from CSS) plus seven new presets (`tabFade`, `dialogIn`, `dialogOut`, `listInsert`, `listRemove`, `phaseReveal`, `badgeCrossfade`); every Svelte transition site MUST import one of these — never hand-roll `{duration, easing}`. New `frontend/src/lib/styles/shared-keyframes.css` hoists `skeleton-pulse` (was duplicated in HistoryPanel + ClusterRow), `scrim-in` + `modal-in` (DestructiveConfirmModal local copies), `fly-in-bottom` (renamed from Toast.svelte's local `toast-in` for reuse) plus three new sites — imported once via `app.css`, four legacy duplicates removed. New `frontend/src/lib/components/shared/AnimatedDialog.svelte` modal scrim + dialog primitive coordinates `transition:fade` (scrim) + `transition:fly` (dialog) through `dialogIn`/`dialogOut` presets, with ESC handler + click-outside dismissibility flag. New `frontend/src/lib/utils/copy-feedback.svelte.ts` exposes `useCopyFlash()` Svelte 5 reactive primitive replacing four hand-rolled `setTimeout` timers (1200/1500/2000ms in Logo/PassthroughView/ForgeArtifact/CodeBlock) with one `--duration-copy-flash` window (1500ms in `app.css`); re-trigger correctly clears prior timers. **Phase 2 — top visible gaps** (10 sites): EditorGroups tab cross-fade (replaced display:none/block snap), SettingsPanel accordion bodies (Provider / Rate limits / System wrapped in `transition:slide={navSlide}`), ClusterNavigator scroll-into-view race fix (waits `navSlide.duration + 20ms` so layout settles before scroll, plus `block:'center'` so partially-visible matches re-center), HistoryPanel row entrance (`in:slide={listInsert}`; `out:slide` deliberately omitted to preserve the surgical-removal SSE contract), TierGuide overlay scrim (snap → coordinated fade via `dialogIn`/`dialogOut`), DestructiveConfirmModal scrim (symmetric fix), Toast exit (`out:fly` with `listRemove` preset), RefinementTimeline collapse, SeedModal result/error/progress reveals. **Phase 3 — secondary gaps + DRY**: brand-aligned animation tokens (`--duration-skeleton` 1500ms + `--duration-stagger` 350ms in `app.css` replace literal magic numbers in ClusterRow/HistoryPanel/TierGuide; ForgeArtifact uses shared `navSlide` preset instead of three `{duration: 200}` overrides). `prefers-reduced-motion` honored throughout via 0.01ms duration override.

### Fixed
- **Write-queue `worker_alive` must require a real `asyncio.Task`** — resolves all 4 backend test failures observed on the v0.4.18 stable release (`test_lifespan_startup_and_shutdown`, `test_lifespan_startup_handler_errors_do_not_crash`, `test_cold_path_returns_correct_counts`, `TestAuditHook::test_install_idempotent_via_explicit_uninstall`). Root cause: the two lifespan tests patch `asyncio.create_task` with a `DummyTask` side effect whose `done()` always returns `False`; because modules share the asyncio module object, the patch replaces `asyncio.create_task` globally and `WriteQueue.start()` wires `_worker_task` to a DummyTask. Pre-fix `worker_alive` reported `True` (DummyTask.done() → False), so lifespan shutdown took the "queue is healthy" branch and `submit()` enqueued work on a queue with no real worker — the awaited Future never resolved and the event loop sat idle in `epoll_wait` until pytest-timeout fired at 60s. The two unrelated failures were collateral damage from the hung lifespan tests leaving `install_read_engine_audit_hook` registered + module-level taxonomy state pre-set when SIGALRM cancellation tore through them. **Fix**: strict `isinstance(self._worker_task, asyncio.Task)` check in `worker_alive`. Production behavior unchanged (`asyncio.create_task` returns `asyncio.Task`); test mode now correctly reports the worker as non-alive when the spawn primitive is intercepted, and lifespan falls through to the migration-mode fallback that uses a direct `async_session_factory` write. 3 new regression tests in `TestLifecycle` pin pre-start, non-Task object, and post-stop semantics.

- **Close 4 SSE reactivity gaps + brand-align rate-limit settings card** — closes the highest-impact gaps surfaced by the global reactivity audit. **Backend — single rate-limit emit boundary**: new `services/rate_limit_state.py:publish_rate_limit_active()` helper is the sole emitter that BOTH writes to the persistent `RateLimitStateStore` AND publishes the `rate_limit_active` SSE event so the store, `GET /api/health`, and the frontend `rateLimitStore` always agree; all emit sites now route through it. `providers/base.py:call_provider_with_retry` emits `rate_limit_active` on every `ProviderRateLimitError` propagation (both wait-exceeds-cap and retries-exhausted branches) — covers EVERY surface using the retry helper (optimize/refine/batch/probe/score/suggest), not just the probe + (sometimes) batch paths that emitted before. Pre-fix a rate-limit during a normal optimize call was silent until the next 60-second health poll. `services/generators/topic_probe_generator.py:_publish_rate_limited` refactored to delegate to the shared helper, eliminating parallel-emitter divergence risk. `routers/github_auth.py` emits `repo_unlinked` when `/me` cleans up a revoked/expired token (pre-fix the UI showed "connected" until the next file-browser action 401'd). **Frontend — wire missing handlers**: `routes/app/+page.svelte` adds handlers for two events the backend already published but the frontend ignored — `optimization_updated` (routes through `'optimization-event'` for surgical row re-fetch instead of waiting for a manual refresh) and `repo_unlinked` (surgical clear of `linkedRepo`/`fileTree`/`branches`/`indexStatus` so GitHubPanel falls back to device-flow auth immediately, without waiting for the cascading `taxonomy_changed` race window). `stores/rate-limit.svelte.ts:activeList` entries gain `source` tag + `detected_at_ms` so SettingsPanel can render operator-grade detail rows; `applyHeuristicFlags` now tags detections as `"heuristic_flags"` so the UI distinguishes them from SSE-driven ones. **Frontend — brand-align Settings rate-limits card**: replaces the wall-of-text "Behavior" data-row with the canonical key-value rhythm used in PROVIDER/DEFAULTS/SYSTEM (long-form behavior paragraph removed → one-line tooltip on accordion header per Voice & Tone "tooltips = one technical line"); switched warning accent from `--color-neon-amber` (not in palette) → `--color-neon-yellow` (#fbbf24, canonical warnings); active state shows `provider → countdown` plus tight `Resets` (HH:MM:SS, dropping ↳-prefixed indented sub-row), `Detected` (relative time), and `Source` rows so operators can correlate which surface caught the 429; clear state shows `Status: clear` + `Fallback: inactive` matching PROVIDER's `API key: not set` rhythm. New regression coverage: 2 tests in `tests/test_providers.py` + autouse rate-limit-store fixture in `tests/test_rate_limit_state.py` (prior to the helper rollout the real `publish_rate_limit_active` from `test_topic_probe_generator.py` leaked provider state across suites and broke `test_health_includes_rate_limit_clear` in alphabetical order) + 4 new tests in `stores/rate-limit.test.ts` pinning `source` threading + `detected_at_ms` exposure.

- **Cold-boot SSE flicker — add `'connecting'` state** — `sseHealthStore.connectionState` defaulted to `'disconnected'` on construction; `StatusBar` renders `connectionState === 'disconnected' ? 'SSE ×' : 'SSE'`, so on every cold page-load the indicator flashed the red × marker until the EventSource `open` event fired (~50–200ms). The connection was healthy the whole time — the flash mis-signalled an error on a fresh boot. Fix: `ConnectionState` gains a fourth value `'connecting'` for the page-load transient with a clear resolution table (`connecting → healthy` on `open`, `connecting → disconnected` on `error`). StatusBar copy + state matrix updated; existing `degraded`/`disconnected` paths unchanged.

- **Routing-tier onboarding-guide trigger — comprehensive consolidation** — collapsed five fragmented trigger call sites (cold-boot effect, two `routing_state_changed` SSE handler branches, two Settings toggles) into a single reactive `$effect` in `frontend/src/routes/app/+page.svelte` that watches `routing.tier` directly. Pre-fix the trigger pathway was brittle: each call site read `routing.tier` at slightly different moments, some awaited preference persistence and others fire-and-forgot, two state mutations (`preferences_changed` SSE, optimistic mutation) flipped the tier silently with no trigger, rate-limit transitions never triggered anything, and module-level `lastTriggeredTier` mutable in `tier-onboarding.svelte.ts` carried HMR-corruption risk. The fragmentation produced multi-cycle bug-after-bug regressions across commits `c561a625` / `f463a9bd` / `ef38edff` that never fully closed the trigger gap. **Root cause** isolated 2026-05-09: the new tier-watching `$effect` was returning early on its first run before reading `routing.tier`, so Svelte 5's dependency tracker never subscribed it to tier changes — subsequent cold-boot tier resolutions (provider detection, force-toggle reconcile, rate-limit flip) silently never re-fired the effect. Compounded by `firstHealthReceived` being a plain `let` (not `$state`), so its flip during `applyHealth()` was non-reactive and never re-triggered the effect either. **Fix layered**: (1) read `routing.tier` unconditionally before the early-return gate so the effect is always subscribed to tier changes; (2) promote `firstHealthReceived` to `$state` for explicit reactivity; (3) `tier-onboarding.svelte.ts:triggerTierGuide()` becomes a thin pass-through (drops module-level `lastTriggeredTier` — dedup moves to consumer-side `lastEffectiveTier` `$state`); (4) Settings toggles `await preferencesStore.setPipelineToggle()` before opening the guide so any tier-derived guide content reads post-mutation state, not stale; (5) backend `PATCH /api/preferences` side-emits `routing_state_changed` via new `RoutingManager.broadcast_external(trigger)` when the patch touches `force_sampling`/`force_passthrough` — closes the multi-client / CLI desync gap where a force-toggle from another origin never propagated to the frontend's guide path. New regression coverage: `frontend/src/routes/app/page.tier-trigger.test.ts` (4 cold-boot tests pinning VS Code closed/open, no-provider, dismissal-respect) + `frontend/src/lib/stores/tier-onboarding.svelte.test.ts` rewritten for pass-through contract (5 tests; mux is now stateless — same-tier dedup lives consumer-side) + `backend/tests/test_preferences_routing_broadcast.py` (8 tests pinning prefs→routing broadcast contract: routing-relevant patches broadcast, non-routing patches don't, broadcast failures don't break the patch). Frontend vitest: 1748 → 1753 passed. Backend pytest: 3656 → 3664 passed. svelte-check: 1067 files / 0 errors / 0 warnings.

### Internal
- **Frontend coverage to 80% lines baseline** — pre-existing baseline: 69.70% lines. Targeted additions across 12 new test files (+1903 LOC) plus WebGL/Three.js exclusions raise the suite to 79.95% lines (effectively 80%) / 76.50% statements / 60.38% branches / 76.90% functions / 1711 tests. New coverage spans: `api/runs.test.ts` (list-runs querystring threading + envelope decoding — regression cover for the 0-vs-undefined param bug + new v0.4.18 endpoint client), `api/seed.test.ts` (POST shape + degraded-path null contracts + `listSeedAgents`), `components/layout/TaxonomyHealthPanel.test.ts` (render + 3-decimal formatting + null fallback + sparkline gating), `components/shared/InternalGuide.test.ts` + `SamplingGuide.test.ts` + `PatternGraphGuide.test.ts` (closed/open + WHY block + step titles), plus 6 more component-level smoke files. WebGL/Three.js paths excluded from coverage (jsdom can't render `three`). No production-code changes — this is pure test infra to lock in a regression baseline.

## v0.4.18 — 2026-05-09

### Added

- **Foundation P3 — Substrate unification** — collapses the asymmetric probe/seed run surfaces into a single unified substrate. New `RunRow` ORM model (18 columns, 4 indexes, 4-value status: `running`/`completed`/`failed`/`partial`) replaces `ProbeRun` and introduces row-state persistence to the seed surface for the first time. New `RunOrchestrator` service (`backend/app/services/run_orchestrator.py`) is the only legitimate writer of `RunRow` rows — owns row creation, status transitions via `WriteQueue.submit()`, cancellation handling under `asyncio.shield()`, exception capture with 2000-char truncation, and `current_run_id` ContextVar lifecycle. Two pluggable generators conforming to a `RunGenerator` Protocol: `TopicProbeGenerator` (refactored from `ProbeService._run_impl`, ~760 LOC, preserves all 5 phases + 8 SSE event types byte-for-byte: `probe_started`, `probe_grounding`, `probe_generating`, `probe_prompt_completed`, `probe_completed`, `probe_failed`, `ProbeRateLimitedEvent`, `rate_limit_active`) and `SeedAgentGenerator` (refactored from `tools/seed.py:handle_seed`, ~330 LOC, preserves early-failure HTTP 200 + `partial` classification + decision events on Channel 2 taxonomy_event_logger). Atomic Alembic migration `58510d3f6b81_add_run_row_table_foundation_p3.py` with matched-state idempotency guard (3-state: fresh / already-migrated / partial-completion abort), 18-column table create + 4 indexes + `INSERT...SELECT` backfill from `probe_run` (`mode='topic_probe'`, scope/commit_sha → `topic_probe_meta` JSON) + drop legacy table; reversible downgrade with NOT NULL re-COALESCE for `topic`/`intent_hint`/`repo_full_name`. Backward-compat preserved across all four surfaces: `POST /api/probes` (race-free SSE via subscribe-before-dispatch + caller-minted `run_id`), `POST /api/seed` (sync semantics + additive `run_id` field on `SeedOutput`), `synthesis_probe` MCP tool (response shape unchanged + bus→ctx progress bridge for AC-C6-3 restoration via `event_bus.subscribe_for_run` filter forwarding `probe_prompt_completed` to `ctx.report_progress`), `synthesis_seed` MCP tool (additive `run_id` only). New unified endpoints `GET /api/runs` + `GET /api/runs/{run_id}` (paginated, mode/status/project_id filters) + `GET /api/seed` + `GET /api/seed/{run_id}` (additive). New `event_bus.subscribe_for_run(run_id)` filtered async iterator with 500ms ring-buffer replay + non-dict-sentinel-safe `__anext__`; reuses existing `_subscribers` set + `_replay_buffer` deque without modifying `publish()`. Frontend `SeedModal.svelte` gains additive `runId?: string | null` prop with truthy-guard filter on `seed-batch-progress` events (default `null` preserves pre-P3 global-progress contract; empty-string also treated as no-filter). `current_run_id` ContextVar rebound at canonical home `probe_common.py` with backward-compat `current_probe_id` alias preserving object identity (`current_run_id is current_probe_id` — pinned by `tests/test_probe_service_module_split_v0_4_17.py:27`). `ProbeRun` ORM class deleted (replaced transient PR1 Python-alias subclass that extracted legacy `scope`/`commit_sha` kwargs into JSON metadata). `ProbeService` class deleted (logic lives in `TopicProbeGenerator`); `probe_service.py` reduced from 2270 → 37 LOC retained as re-export shim for `current_probe_id` backward compat. 51 `ProbeRun` references in 8 test files rewritten to `RunRow`; 2 obsolete test files deleted (`test_probe_service.py` + `test_probe_service_queue_routing.py`, ~2210 LOC retired in tests; coverage moved to `test_run_orchestrator.py` + `test_topic_probe_generator.py`). `_gc_orphan_runs(db) -> int` helper sweeps both modes (preserves legacy `(db) -> int` signature, composes inside `run_startup_gc._do_sweep` batched commit); legacy `_gc_orphan_probe_runs` becomes no-op returning 0. Generator dispatch table on `app.state.run_orchestrator` (FastAPI process) + parallel registration on `mcp_server.py` lifespan (MCP process) via `_set_run_orchestrator` / `get_run_orchestrator` helpers in `tools/_shared.py`. 16 cycles + 2 prework cycles + 1 follow-up cycle = 95 commits across `release/v0.4.18`. ~94 net new tests across 12 categories per spec § 9 (cat 1 RunRow + migration: 8 tests; cat 2 RunOrchestrator lifecycle: 14; cat 3 RunGenerator protocol: 7; cat 4 TopicProbeGenerator: 14; cat 5 SeedAgentGenerator: 10; cat 6 probes shim: 13; cat 7 seed shim: 8; cat 8 MCP tools: 7; cat 9 /api/runs: 7; cat 10 GC sweep: 4; cat 11 cross-process correlation: 4; cat 12 ProbeRun ORM removal: 6). Backend suite: 3648 passed / 4 skipped (pre-existing `test_main.py` 2-test hang verified pre-existing on parent commit `2bb8de9a`). Frontend suite: 1557 passed across 97 files. `alembic check` clean. ruff + svelte-check clean.

- **Foundation P3 — Spec + plan + 5 review cycles each, all APPROVED-ZERO via independent subagents** — `docs/superpowers/specs/2026-05-06-foundation-p3-substrate-unification-design.md` ran 5 spec-review cycles (V1→V5) against independent general-purpose subagents until verdict APPROVED-ZERO; V1 surfaced 8 critical issues including the original incorrect "collapse SeedRun + ProbeRun" framing (verified no `SeedRun` model has ever existed — only `ProbeRun` did, with 17 columns), V2 surfaced 8 spec-text issues, V3-V5 cleared remaining staleness. `docs/superpowers/plans/2026-05-06-foundation-p3-substrate-unification.md` (~5400 LOC, 16 cycles + Cycle 3.5 fixture cycle) ran 5 plan-review cycles until APPROVED-ZERO; V1 surfaced 8 critical issues incl. INTEGRATE/OPERATE phase absence, Cycle 11/12 stub `pass` test bodies, `event_bus.subscribe_for_run` shape incompatibility with actual `EventBus` class, SQLAlchemy STI broken under chosen option (b); V2-V5 cleared remaining issues. No self-review used at any review cycle.

- **Foundation P3 prework (pre-Cycle 1)** — Pre-existing Alembic schema drift cleanup + lifespan-DDL consolidation. See `### Changed` + `### Fixed` below.

### Changed
- **Lifespan-DDL consolidation into Alembic (Foundation P3 prework)** — migrated all ~12 startup-time `ALTER TABLE` / `CREATE INDEX` / `CREATE TABLE IF NOT EXISTS` blocks out of `app/main.py` lifespan into a single forward-only Alembic migration `bdd8e96cf489_consolidate_lifespan_ddl_into_alembic.py`. Closes the audit-trail gap where `alembic upgrade head` told only half the story and lifespan finished the job — all schema changes now live in `alembic/versions/`. Migrated columns: `optimizations.{routing_tier, optimized_embedding, transformation_embedding, phase_weights_json}`, `meta_patterns.global_source_count`, `optimization_patterns.global_pattern_id`, `prompt_cluster.weighted_member_sum`, `linked_repos.project_node_id`, `repo_index_meta.{explore_synthesis, synthesis_status, synthesis_error}`, `repo_file_index.content`. Migrated indexes: `ix_optimizations_project_id`, `ix_taxonomy_snapshot_created_at`, `idx_repo_file_index_repo_branch_path` (unique). Migrated DML: `routing_tier` backfill (3 stages by provider), `synthesis_status='ready'` backfill where `explore_synthesis IS NOT NULL`. Type fidelity restored on fresh DBs: `phase_weights_json` declared as `JSON` (was `TEXT` in lifespan hook) — `compare_type` callback in `env.py` continues to forgive legacy DB drift. `prompt_cluster` column add uses raw `ALTER TABLE ADD COLUMN` to avoid `op.batch_alter_table` dropping the COALESCE-based `uq_prompt_cluster_domain_label` partial index that SQLAlchemy can't reflect. `main.py` shrinks from 2204 → 1957 lines (–264 lines net). New regression file `tests/test_main_lifespan_no_ddl.py` (19 tests) pins both the static-text invariant (no DDL keywords inside lifespan) and per-(table,column) schema existence on a freshly migrated DB.
...
> *Retroactively tagged on 2026-05-05. The fix was originally completed and merged on 2026-05-04 but `./scripts/release.sh` was never run because v0.4.16 Foundation P1 work began immediately afterwards. The tag (created 2026-05-05) restores the originally-scoped release boundary so the version history matches the work decomposition: v0.4.15 = standalone P0 fix; v0.4.16 = Foundation P1.*
...
- **v0.4.12 — SSE slow-poll fallback + visibility recovery** — pre-fix, after `MAX_RETRIES=10` exponential-backoff attempts (~111s total), the SSE store entered a permanent "Retries exhausted" state — only a manual page reload could recover the connection, leaving the UI stranded in a disconnected state on every multi-restart dev workflow. Now drops into a slow-poll cadence (`SLOW_POLL_DELAY_MS=30000`, same jitter). `retryCapped=true` is now a SIGNAL for the tooltip ("SSE disconnected · Slow-poll retry in Xs"), not a termination state. Visibility-change handler retries immediately when the tab regains focus and SSE is disconnected.
...
- **`prompts/optimize.md` prompt-aware rationale guideline** — the optimizer was producing generic "Audit alone is ambiguous" rationales instead of crediting cues already present in the input prompt. New guideline drives optimizer to cite specific cues (verify/Confirm/check) from the input prompt when explaining changes. (Hot-reloaded.)
...
### Added
- **Self-update hardening — pre-flight endpoint + drain lock + auto-stash + per-step progress (PR upcoming)** — comprehensive rework of the auto-update flow against three P0 risks identified in the end-to-end audit: (1) **strategy edits silently lost** when `git checkout --force` runs after the dirty-tree error; (2) **in-flight optimization race** during restart corrupting the partially-written DB row; (3) **local commits ahead of origin/main silently orphaned** by pure-tag checkout. Six new pieces ship together: (a) `customization_tracker.py` records every `PUT /api/strategies/{name}` edit into `data/.user_customizations.json` (gitignored, atomic-write, schema-versioned, defensively coerces corrupt registries to empty); (b) `GET /api/update/preflight` returns a `PreflightStatus` with dirty-file source classification (`user_api` / `manual_edit` / `untracked`), commits-ahead-of-origin, in-flight optimization count, detached-HEAD detection, target-tag presence — frontend calls this BEFORE enabling the apply button so the dialog renders the full picture; (c) `UpdateInflightTracker` singleton coordinates pipeline `begin/end(trace_id)` with `apply_update`'s drain wait (60s budget, 0.5s poll), and the `optimize` router returns 503 when `update_in_progress=True` so new requests don't get killed mid-pipeline; (d) **auto-stash + pop** in `apply_update` — `git stash push -m synthesis-update-{tag} prompts/` before checkout, `git stash pop` (matched by prefix sentinel) after, with stash-pop conflicts surfaced via `update_complete.stash_pop_conflicts` so the dialog enumerates exactly which files need manual resolution; (e) **per-step SSE events** — `update_step` emitted at every phase (`preflight / drain / fetch_tags / stash / checkout / deps / migrate / pop_stash / restart / validate`) with `running / done / warning / failed` status — frontend renders a per-step timeline replacing the unused `updateStep` placeholder; (f) **branch-ahead + detached-HEAD warnings** in preflight; **`force=true`** on `apply` bypasses warnings (commits-ahead, in-flight remaining after drain) but never blocking issues (non-prompt uncommitted changes, invalid tag). Frontend rebuild: `UpdateBadge` dialog now renders the preflight panel with `dirty-source` color-coded chips, dirty-file paths in a collapsed details, in-flight optimization counter, auto-stash tag; the apply button is gated on `can_apply`; warnings get a "Update & Restart (force)" variant; updating-state badge shows the current step name; new `update-dialog.completion` view renders `validationChecks` (was populated but never displayed) + `stashPopConflicts` recovery list; new retry button after the 120s health-poll timeout instead of leaving the dialog stranded. 17 customization-tracker unit tests + 17 update_service tests (drain lifecycle, preflight readiness gates, auto-stash sentinel, round-trip stash preservation, concurrent-update block) cover the new surface. Backend full suite 3094 passed.
...
- **Observability separation-of-concerns sweep — `ActivityPanel` (topology terminal) vs `DomainLifecycleTimeline` (Observatory)** — both surfaces consumed `clustersStore.activityEvents` and shared filter/summary logic; a 4-item audit (`docs/observability-separation-audit.md` thread) found drift between the two and one cross-surface side-effect. Each fix lands behind a regression test:
  ...
  - **Timeline `OP_TO_FAMILY` rewritten over the actual backend op vocabulary** — replaced 7 dead entries with the 7 lifecycle ops the backend genuinely emits (`archive`, `candidate`, `emerge`, `extract`, `state_change`, `template_lifecycle`, `recovery`). Pre-fix these 7 fell through to the "uncategorised" bucket and only rendered when every family chip was on; the Observatory's lifecycle timeline could not meaningfully filter by family.
...
- **Live Pattern Intelligence Tier 1 — `ContextPanel.svelte`** (ADR-007 first iteration) — replaces the single-banner `PatternSuggestion.svelte` with a persistent sidebar mounted by `EditorGroups.svelte` alongside the prompt editor. Reads `clustersStore.suggestion` and renders matched cluster identity (label + similarity % + match_level + domain dot), meta-patterns checkboxes, and a separately-bordered GLOBAL section for cross-cluster patterns. APPLY button writes the active selection to `forgeStore.appliedPatternIds` for downstream pipeline injection. Selection persists across applies (panel doesn't auto-dismiss). Handles three transient states: `_matchInFlight` fades the body to 0.5 opacity, `_matchError === 'network'` draws a 1px neon-red inset contour on the header, `_lastMatchedText !== ''` switches the empty-state copy from "waiting for prompt" to "no similar clusters found". Collapse/expand chevron rotates 90° via the brand `--duration-micro` + `--ease-spring` motion, mirroring `CollapsibleSectionHeader.svelte`'s navigator section toggles. Open/closed state persists to `localStorage['synthesis:context_panel_open']`. At < 1400px viewport the panel mounts as a 28px rail by default (expand via chevron; preference persists). Mount-gated to `editorStore.activeTab?.type === 'prompt'` so result/diff/mindmap views stay clean. Hidden during synthesis (`forgeStore.status ∈ {analyzing, optimizing, scoring}`). Full a11y (`role=complementary`, `aria-label`, `aria-expanded`, `aria-controls`) + `prefers-reduced-motion` collapses transitions to 0.01ms. 30 ContextPanel tests + 3 EditorGroups regression tests (I3 + I4 + tab-gating) + 2 PromptEdit regression locks (I1 + I2).
...
- **`.toast-stack` z-index raised above modal layer** — initially placed at `z-index: 800`, which is below `DestructiveConfirmModal`'s scrim (900) and panel (901). A single-row delete toast opening during the grace window could become unreachable when a bulk-confirm modal opened over it. Raised to `z-index: 1100` so the pre-commit toast + its Undo button stay interactive even when a modal is stacked above (still below the 9999 tier reserved for CommandPalette / SeedModal).
- **Row delete button is keyboard-accessible** — the hover-reveal `× ` affordance initially used `tabindex="-1"`, making it unreachable via tab. Changed to `tabindex="0"`; the existing `.row-item:focus-within .row-delete-btn { opacity: 0.6 }` rule reveals the button when the parent row receives keyboard focus, and the button's own `:focus-visible` outline provides the focus indicator. Keyboard navigation now supports Tab → row → Tab → × → Enter to delete.
...
- **GC hardens archived-zero-member pruning for rows with NULL `archived_at` (I-2)** — `warm_phases::phase_reconcile` now matches archived rows missing the `archived_at` timestamp via `updated_at < NOW() - 24h` fallback. Prevents the specific debris observed in production (archived `backend`/`frontend` nodes with NULL `archived_at` that survived GC indefinitely because the `> 24h` clause short-circuited on the NULL).
...
- **`domain_readiness_notifications.enabled` default flipped to `true`** — PR #27 shipped the feature gated off-by-default with no UI toggle, which rendered the entire tier-crossing toast pipeline unreachable in a fresh install. Defaults now mirror the new master mute button semantics: on by default, opt-out via header bell or per-row bells. Regression test added for opted-out preferences carrying through the default flip.
...
### Added
- **Live pattern detection on typing** — two-path detection replaces paste-only system: typing path (800ms debounce, 30-char min) + paste path (300ms, 30-char delta). Patterns now surface as input is typed, not just on paste. AbortController cancels in-flight requests. Persistent chip bar below textarea confirms applied patterns
...
- **Template preview card** — inline expandable card in ClusterNavigator showing pattern texts, best prompt excerpt, score. Two actions: "Load Prompt + Patterns" (full template load) and "Apply Patterns Only" (keep the active prompt, inject template patterns)
...
- **Auto-injection runs alongside explicit patterns** — `auto_inject_patterns()` now fires even when explicit `applied_pattern_ids` are set, merging both sources. Previously, explicit selection completely disabled auto-injection
...
### Added
- **GitHub Device Flow OAuth** — zero-config authentication using hardcoded GitHub App client ID. No client secret or callback URL required. Gated handoff UX: shows device code first, then a click opens GitHub
...
### Fixed
- **Gated device flow handoff** — auto-opened GitHub tab before showing device code. Now shows code first, with an explicit button click to proceed
...
### Fixed
- **Activity panel showed only 2 events after restart** — JSONL fallback only triggered when ring buffer was completely empty (0 events). Two warm-path events prevented fallback, leaving the panel with zero historical context
...
- Unified domain taxonomy — domains are now first-class taxonomy nodes discovered organically from optimization usage signals (ADR-004)
...
- Added toast notification on failed session restore — surfaces "Previous session could not be restored" instead of a silent empty state
