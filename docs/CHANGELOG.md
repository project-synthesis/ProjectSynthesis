...
- **`e2e_test_workflow_delete.py` — end-to-end delete-flow harness** — new repo-root script exercises the full delete path against a live backend: creates an optimization via `POST /api/optimize`, verifies it lands in `GET /api/optimizations`, bulk-deletes via `POST /api/optimizations/delete`, and asserts it's gone from history plus the SSE stream carried an `optimization_deleted` event. Complements `e2e_test_workflow.py` (internal-tier) and `e2e_sampling_workflow.py` (sampling-tier).
...
- **Test isolation for rate-limit + event-bus state** — `conftest.py` exposes a public `reset_rate_limit_storage()` fixture that clears the in-memory SlowAPI rate-limit bucket between tests (previously a private helper accessed via module spelunking), and a shared `drain_event_bus()` that clears queued SSE events + resets `_shutting_down`. Prevents leakage from lifespan-tests (which set `_shutting_down=True` and never reset it) into later tests that expect a live event bus — caught while the bulk-delete event-emission test was flaking under parallel `-n auto` runs.
