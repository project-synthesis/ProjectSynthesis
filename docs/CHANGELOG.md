...
### Fixed
- **Template spheres inherit domain color instead of neon cyan override** — `stateNodeColor()` used to force every `state='template'` sphere to `stateColor('template')` (#00e5ff), which hid the template's domain membership entirely: a security-domain template and a backend-domain template rendered identically. Template identification is already carried by the shader-based starfield decoration on the sphere, the cyan TEMPLATE badge in the Inspector, and the dedicated TPL state-filter tab, so the color override was redundant and hid brand partitioning. Templates now inherit their domain color via `taxonomyColor()`. The sub-domain→parent walk in `buildSceneData` was broadened from `isSubDomain`-only to any node whose `parent_id` is a domain node, so a template under `security > token-ops` now renders in security red (`#ff2255`) instead of cyan or the token-ops OKLab variant. Clusters parented to a sub-domain inherit the top-level domain color by the same mechanism. 2 new Vitest cases lock (a) per-domain template color differentiation and (b) template-under-sub-domain top-level inheritance; the legacy `#00e5ff` assertion was removed.
