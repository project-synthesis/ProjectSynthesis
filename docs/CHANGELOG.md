...
### Changed
- **Pattern Graph 3D — depth pass + brand-compliance regression gates** — closes the gap between the 3D Pattern Graph code and the new SKILL.md "3D Visualization Scope" + `references/3d-visualization.md` rules. Six TDD cycles, six regression gates added. **Cycle 1 — `ClusterPhysics` spring physics**: replaces the linear `Math.pow(0.01, delta/0.5)` lerp with semi-implicit Euler integration (`k=120`, `d=12`, `dt clamp=0.1s`, `velocityFloor=1e-4`). Underdamped spring (ζ ≈ 0.55) overshoots before settling; velocity-floor snap-to-target prevents infinite micro-oscillation at rest and makes equality assertions possible (`finalScale === 1.02`, not `1.01999…`). 3 new tests asserting overshoot, exact snap, and `dt`-clamp NaN safety. **Cycle 2 — brand-compliance gate**: `frontend/src/lib/components/taxonomy/brand-compliance.test.ts` enforces zero banned post-processing imports (`UnrealBloomPass` etc.) and zero banned-word matches (`\b(glow|radiance|bloom|halo)\b`, PCRE2 word-boundary, case-insensitive) across 11 in-scope files. Whitelist exception scoped to test files asserting absence inside string/regex literals. Same cycle: 14-identifier rename pass `_haloPool` → `_templateRingPool`, `_haloById` → `_templateRingById`, etc., across `SemanticTopology.svelte` (source) + `SemanticTopology.test.ts` (matched-pair). Cyan template indicator ring stays — only banned terminology goes. **Cycle 3 — per-frame allocation budget**: module-level scratch table at the top of `SemanticTopology.svelte`'s `<script>` (`_scratchVec3a`, `_scratchQuat`, `_scratchColor`, `Z_AXIS`) — borrow rules documented inline. New `perf-budget.test.ts` wraps `THREE.Vector3` / `Color` / `Quaternion` / `Matrix4` constructors via `vi.mock('three', importOriginal)` selective re-export with shared `_counters` object; runtime gate asserts `BeamPool.update` and `ClusterPhysics.update` allocate zero THREE primitives across 100 simulated frames at 60fps. Counter mechanism validated by introducing a deliberate `new THREE.Vector3()` regression and confirming the test caught exactly 100 allocations (one per frame), then reverting. **Cycle 4 — cleanup-contract regression gate**: `cleanup-contract.test.ts` source-greps `SemanticTopology.svelte` for 9 walked-back banned identifiers absent (`_removeDustAnim`, `_breathingAnim`, `_removeEdgeAnim`, `__semTopHaloPool`, `_haloPool`, `_haloById`, `_haloGroup`, `_freeHalos`, `HALO_POOL_*`, `_acquireHalo`, `_releaseHalo`, `__semTopGlowTexture`), 4 renamed identifiers present, all 5 surviving cancellers wired in cleanup body (`removeBeamUpdate`, `_removeRingLodUpdate`, `_removeFormationAnim?.()`, `_removeDomainRotation?.()`, `_removeReadinessBillboard?.()`), all 8 disposable releases (`beamPool?.dispose()`, `clusterPhysics?.clear()`, `disposeRingEntry(entry)`, `_readinessRings.clear()`, `ro.disconnect()`, `interaction?.dispose()`, `labels?.dispose()`, `renderer?.dispose()`), and that every `globalThis.X = …` assignment is gated by `import.meta.env.MODE === 'test'`. Cleanup body extracted via balanced-brace walk from the last `return () => { … }` before `</script>`. **Cycle 5 — `MeshStandardMaterial` + 3-light setup**: cluster + domain anchor fills swap from `MeshBasicMaterial` (which bypasses lighting and produces flat un-emissive surfaces) to `MeshStandardMaterial` with `roughness=0.6`, `metalness=0.0`, `emissive` = domain hex, `emissiveIntensity` data-driven (cluster: `0.6 + 0.8 * clamp((memberCount-1) / 49, 0, 1)`; domain anchor: `0.4` recessed). `castShadow=true` on cluster fills. `TopologyRenderer` constructor enables shadow map with `THREE.PCFShadowMap` (canonical replacement for the deprecated `PCFSoftShadowMap`) and adds three lights per brand reference: `AmbientLight(0xffffff, 0.3)`, `DirectionalLight(0xffffff, 0.7)` at `(5, 10, 5)` with `castShadow=true` + `shadow.mapSize.set(1024, 1024)`, `HemisphereLight(0x1a1a2e, 0x06060c, 0.2)` (sky matches `DEFAULT_BG`). Readiness ring stays `MeshBasicMaterial` per the depth-write-false banner-overlay carve-out. New `material-lighting-baseline.test.ts` source-greps every brand-canonical setting. **Cycle 6 — adaptive-distance + zero-length focus guard**: `focus-math.ts` extracts `computeFocusEndpoint(startPos, startTarget, target, distance, opts)` as a pure function. Clamps requested distance to `[controls.minDistance, controls.maxDistance]`; detects zero-length view direction (`startPos ≈ startTarget`, length below `1e-10`) and substitutes `+Z` fallback so the final camera position has `Number.isFinite` on every component. Pre-fix `subVectors(startPos, startTarget).normalize()` produced a NaN cascade through `multiplyScalar` + `add` into `endPos`, silently destroying camera state on the first focus-on-current-target call. 8 unit tests on the helper (real Three.js, no mock); `focusOn` glue stays in `TopologyRenderer.ts`. **Validation**: 1788/1788 frontend tests pass (cycle-2 brand gate + cycle-3 perf gate + cycle-4 cleanup gate + cycle-5 material-lighting gate + cycle-6 focus-math tests = +24 net regression coverage). svelte-check 0 errors / 0 warnings. brand-compliance gate confirms zero banned-word matches across 12 in-scope files. Spec at `docs/superpowers/specs/2026-05-09-pattern-graph-depth-design.md` (APPROVED-ZERO V4 after 4 independent-subagent review cycles). Foundation for any future Pattern Graph 3D work — every regression now caught at the static-analysis or runtime-counter layer.
