...
- **Provider threaded through enrichment + tool handlers** — `HeuristicAnalyzer.analyze()` and `ContextEnrichmentService.enrich()` now accept a `provider` kwarg at every call site (`optimize` router, `refinement` router, `batch_pipeline`, `pipeline_phases`, `refinement_service`, `tools/optimize`, `tools/prepare`, `tools/refine`, `tools/save_result`). The A4 confidence-gated Haiku fallback can now resolve the correct provider instance without a global lookup — eliminates a class of race conditions in sampling and passthrough tiers where `app.state.routing` was still settling.
...
- **Faithfulness heuristic normalization for expansion strategies (I-5)** — `heuristic_faithfulness()` accepts an optional `strategy_used` kwarg; meta-prompting / role-playing / chain-of-thought / tree-of-thought / few-shot strategies are flagged as `EXPANSION_STRATEGIES` and get a conservative dampener (40% of the gap toward 8.5 when raw cosine score < 7.0) so the hybrid scorer no longer systematically under-counts faithfulness on outputs that are *meant* to expand. Direct strategies keep the full cosine penalty. `score_prompt()`, `pipeline_phases`, `sampling_pipeline`, `batch_pipeline`, `refinement_service` thread the strategy through.
