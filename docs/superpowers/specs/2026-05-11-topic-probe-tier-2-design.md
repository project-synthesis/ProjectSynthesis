...
| `services/generators/_constants.py` | **DEFERRED to T3** — originally planned NEW file housing `PROBE_PROMPT_CONCURRENCY` for cross-generator parallelization. Per strict `feedback_tdd_protocol.md` "Scaffolding ≠ TDD" canon (line 334-336: pure constants without a failing consumer test belong in the consumer cycle), this constant is dropped from T2 entirely. T3 introduces both the constant AND its first consumer (concurrent per-prompt execution) together. T2 ships replay as sequential per-iteration without any concurrency-related constants. |
...
- Emitted **by the helper itself** (added to every call's output): `task_type_distribution: dict[str, int]` (new contract extension; T1 `_build_aggregate` did not compute this). The helper aggregates `r.get("task_type", "unknown")` defensively — topic_probe per-prompt dicts (current `topic_probe_generator.py:392-404`) lack a `task_type` key, so topic_probe runs would emit `task_type_distribution={"unknown": N}`. Replay's per-prompt dicts (per the spec body at §5) include `"task_type": pending.task_type`, so replays emit a populated distribution. Future T3+ work that wants a populated topic_probe distribution can extend `_run_one_prompt` to include `task_type` then; T2 ships the defensive helper unchanged either way.
...
| `routers/probes.py` | EXTEND — `grounding_mode` body field on `ProbeRunRequest`; `Prefer: respond-async` header → 202 |
...
| Frontend `src/app.css` (EXTEND) | **NEW `@keyframes forge-spark`** — T2 surfaces (Save-Suite click, emergent taxonomy node entry, RegressionBadge nominal→firing transition) reference this animation but it is NOT yet shipped in `frontend/src/app.css` (verified by grep). Definition: `0%: scale(1) rotate(0)`; `25%: scale(1.2) rotate(3deg)` + yellow flash via `background-color`/`color: var(--color-neon-yellow)`; `100%: scale(1) rotate(0)`. Single-shot ~250ms `ease-out`. Brand canon at `component-patterns.md:223` describes the effect (`Yellow flash + scale(1.2) + rotation`) but lists `Duration: varies` — T2 fixes 250ms as the shipped value and updates `component-patterns.md:223` to record it (paralleling the `copy-flash` 600ms→1500ms and `StatusBar` 22px→20px doc-sync reconciliations). |
...
`suite_repo_drift` is **informational, not an error** — detected inside `ReplayRunGenerator.run()` and surfaced via `RunRow.aggregate["replay_warnings"]` after the replay completes (frontend reads it from `GET /api/runs/{run_id}.aggregate`, not from the immediate `POST /api/suites/{id}/replay` 202 response which returns before generator execution). The earlier `ReplayRunOut.warnings` field idea is removed — it was structurally unpopulatable at 202 dispatch time.
...
class ReplayRunOut(BaseModel):
    ...
    NOTE: this response is constructed BEFORE the replay generator runs
    (dispatch_async returns immediately after the initial RunRow INSERT,
    long before per-prompt execution begins). Repo-drift detection lives
    inside ReplayRunGenerator.run() and is written to RunRow.aggregate
    [\"replay_warnings\"] post-completion — NOT to this response payload.

    Frontend consumers polling GET /api/runs/{run_id} read the eventual
    `aggregate.replay_warnings` (after status='completed'), NOT this
        ...
        for idx, item in enumerate(suite_snapshot.prompts_snapshot):
            try:
                ...
                prompt_results.append({
                    ...
                    # `r["overall_score"]`). Replay must use the same key as topic_probe's
                    # per-prompt dict (topic_probe_generator.py:396 emits "overall_score":7.0)
                    # so the helper returns a valid mean_overall instead of None.
                ...
                event_bus.publish("probe_prompt_completed", {
                    ...
                    "overall_score": overall, "delta": overall - baseline_overall,
                })
...
**Concurrency note:** the spec ships replay as **sequential per-prompt** (try/except per iteration, matching `TopicProbeGenerator` Phase 3). **T2 introduces no concurrency-related constants** — `PROBE_PROMPT_CONCURRENCY` and `services/generators/_constants.py` are fully deferred to T3, where they land together with the first per-prompt concurrent consumer (per `feedback_tdd_protocol.md` "Scaffolding ≠ TDD" canon — pure constants without a T2 consumer belong in T3's cycle).
...
async def dispatch_async(
    ...
    """Spawn-and-return entry point. Awaits the initial INSERT only; the
    spawned task runs the generator + persists terminal status. Optional
    done_event is set by the spawned task when terminal status is reached
    (success / failure / cancellation) — used by run() to gate "blocks
    until terminal" callers. 202 callers pass done_event=None (default).
    """
    ...
    asyncio.create_task(self._run_to_completion(
        mode=mode, request=request, run_id=run_id, done_event=done_event,
    ))

async def _run_to_completion(
    self, *, mode, request, run_id,
    done_event: asyncio.Event | None = None,
) -> None:
...
| Tab cell (TOPIC PROBE) | — (inherits bar height h-7) | px-1.5 | gap-0.5 | text-[11px] uppercase `font-mono` letter-spacing 0.05em — matches the shipped `.seed-tab` typography at `SeedModal.svelte:545-557` (`font-family: var(--font-mono)` + `letter-spacing: 0.05em`); the new TOPIC PROBE tab sits in the same tab bar as GENERATE/PROVIDE and must visually unify. Canonical `font-display` + `letter-spacing: 0.1em` migration of all 3 tabs is deferred to T4's SeedModal consolidation per §14 collapse-math. |
...
**Replay completion events:** replay runs reuse the EXISTING `probe_prompt_completed` (per-prompt) and `probe_completed` (terminal) event types — same payload shapes as topic probe. SSE consumers do not need to learn a new event family. SuitesPanel + RegressionBadge filter polled `RunRow.mode == 'replay_run'` state to surface replay-specific UI; they do not subscribe to a replay-specific event type. **Total `probe_*` event count stays at 7 (6 existing + `probe_warning`).** The 6 existing events are: `probe_started`, `probe_grounding`, `probe_generating`, `probe_prompt_completed`, `probe_completed`, `probe_failed` (verified via grep of `event_bus.publish.*probe_` in `services/generators/topic_probe_generator.py`). The `probe_taxonomy_change` event documented in `backend/CLAUDE.md` line 143 is **never emitted** by current code — that doc claim is stale; pre-release item 11 adds a backend/CLAUDE.md correction to drop it.
...
| 4 | `.compute_regression_alarm` + 30s TTL cache + `regression_alarm_transition` event | 7 | A3 (Python-side filter matches canonical alarm-evaluation logic; cite the spec's SQL + Python filter as the canonical reference) · A1 (alarm-query result columns ALL consumed — none silently dropped) | **O4** (alarm query is a READ path; under concurrent `replay_run` writes the verification is the O4 "background-task race conditions visible only under live load" anti-pattern per `feedback_tdd_protocol.md`, NOT O2 which targets multi-row write batches — race-free snapshot via WAL; verify with concurrent writer harness) · O6 (cross-process replay writes from MCP do not corrupt alarm state) |
| **B — REST + Generators (5-9)** | | | | |
| 5 | `routers/suites.py` (6 endpoints, rate limits, error envelopes) | 15 | A2 (use `WriteQueue.submit()` indirectly via `ValidationSuiteService` — do NOT re-implement persistence in the router) · A4 (response payloads populate all `ValidationSuiteOut` / `ReplayRunOut` / `RunListResponse` fields — no NULL leakage from JOIN queries). **GREEN-step replay-endpoint shape**: Cycle 5 ships `POST /api/suites/{id}/replay` returning 202 with the `ReplayRunOut` body, but the handler body uses a **placeholder dispatch path** — it INSERTs the initial `RunRow(mode='replay_run', status='running', suite_id=...)` via `WriteQueue.submit()` and returns 202 immediately, **without spawning the generator**. The orphan `running` row is reconciled by the existing `_gc_orphan_runs` sweep within 1h TTL until Cycle 6 (registers `ReplayRunGenerator` in lifespan dict) + Cycle 8 (lands `dispatch_async`) together replace the placeholder dispatch with the real spawn primitive. Cycle 5 RED tests assert the 202 response shape + initial INSERT + rate limits — but explicitly DO NOT assert end-to-end replay completion (that lands as a Cycle 8 integration test). | **OPERATE (scoped — replay-endpoint end-to-end checks deferred to Cycle 8) — replay-endpoint end-to-end execution is deferred to Cycle 8** (when `dispatch_async()` + ReplayRunGenerator registration both exist). Cycle 5 OPERATE covers: O5 (client disconnect mid-request for save-as-suite/retire — no orphan rows on rate-limit-rejected writes) · **O8 partial** (replay endpoint placeholder leaves `RunRow(status='running')` that `_gc_orphan_runs` must reconcile within TTL — verify the GC sweep handles the placeholder rows correctly; this is the "orphan-by-design until Cycle 8 fills in" check) · **O4** (concurrent listings during active write_queue traffic — read-side race). The deferred replay-endpoint end-to-end O5+O8 checks (client disconnects mid-real-replay + spawned-task survives connection close) move to Cycle 8's OPERATE focus where the spawn primitive exists. Mirrors the peer-plan v0.4.16-p1a Cycle 1 OPERATE-light precedent (peer-plan Task 1.5, lines 466-503 — "Mandate: Cycle 1 OPERATE is light — full concurrent peer-writer load test belongs to Cycle 2"). |
...
| 9 | `/api/health` regression_alarm block | 5 | A4 (`RegressionAlarmEntry` populates all required fields from joined query — no NULL leakage; column-by-column diff against the join-SQL result columns in spec §5) | **O4** (alarm-query READ path coexists with active replays — race-free snapshot per `feedback_tdd_protocol.md` O4 anti-pattern definition (background-task race conditions visible only under live load), NOT O2; verify with 30 concurrent `/api/health` polls during an active replay) |
| **C — MCP (10)** | | | | |
...
| 12 | SuitesPanel + SuiteRow + SuiteDetailView + RegressionBadge + SuitesStore | 14 | A3 (suite list rows render every field of `ValidationSuiteListItem`; SuiteDetailView consumes full `ValidationSuiteOut` including `baseline_scores.per_prompt`; replay-warning surfacing reads `aggregate.replay_warnings` from `GET /api/runs/{id}` not from the 202 dispatch response — see §4 `suite_repo_drift` clarification) | O1 (browser smoke: SuitesPanel renders for empty / nominal / firing states; RegressionBadge transitions correctly on SSE-emitted `regression_alarm_transition` events) |
...
| `services/generators/_constants.py::PROBE_PROMPT_CONCURRENCY` | **DEFERRED to T3 entirely** — per `feedback_tdd_protocol.md` "Scaffolding ≠ TDD" canon (line 334-336), this constant ships in T3 alongside its first concurrent-consumer cycle. T2 ships zero concurrency-related constants; replay is sequential per-iteration with no semaphore. |
...
11. Stale-doc fix: `backend/CLAUDE.md` T2/T3/T4 minor numbers harmonized with ROADMAP; `backend/CLAUDE.md` line 143 `probe_taxonomy_change` event reference removed (verified by grep: this event is **never emitted** by current code; only 6 `probe_*` events fire in `topic_probe_generator.py:314, 517, 547, 580, 614, 658` = `probe_started`/`probe_grounding`/`probe_generating`/`probe_prompt_completed`/`probe_completed`/`probe_failed`); `docs/ROADMAP.md` line 333 (`POST /api/probes/{id}/replay`) updated to reflect the relocated endpoint `POST /api/suites/{suite_id}/replay`; `.claude/skills/brand-guidelines/SKILL.md` lines 79/214/375 (line 395 is a prose scope statement without an enumerated directory list — exclude from substitution) extended from 5-directory to 7-directory scope; `.claude/skills/brand-guidelines/SKILL.md` line 140 StatusBar height updated 22px → 20px per shipped reality; `.claude/skills/brand-guidelines/references/component-patterns.md` line 219 `copy-flash` updated from 600ms to shipped 1500ms; `.claude/skills/brand-guidelines/references/component-patterns.md` line 223 `forge-spark` duration changed from `varies` to shipped `250ms` (per the NEW `@keyframes forge-spark` introduced in `frontend/src/app.css`). **Cycle assignment for these doc-sync changes**: land in the same PR as Cycle 12 (frontend SuitesPanel/RegressionBadge) since the canon updates live adjacent to the new consumers — keeps doc + impl in lock-step.
