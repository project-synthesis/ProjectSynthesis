...
- `backend/app/models.py` — add `RunRow`. **`ProbeRun` retained as a backward-compat alias for PR1.** Two implementation options, with their tradeoffs:
  ...
  **Implementation picks (a)** for defense-in-depth correctness; option (b) is documented only as a fallback if SQLAlchemy mapper awkwardness blocks (a). PR2 deletes whichever alias was used.
