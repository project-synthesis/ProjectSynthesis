...
**Soak gate execution (SG-2026-05-11, 2026-05-12 → 2026-05-18):** 15 findings surfaced + addressed across the soak window — 2 HIGH + 4 MED + 7 LOW + 2 architectural improvements. Day 1 caught the v0.4.22 audit-hook RAISE-flip blockers (`passthrough_save` / `update_optimization` read-session mutations + 5 missing `app.tools._shared` singletons in lifespan + 3 empty-commit-closure endpoints + refinement rollback FK seeding). Day 4 supplementary surfaced a HIGH test-infrastructure dead-coverage gap on the audit-hook fixture (silently vacuous `caplog` assertions across all 4 prior regression tests), a HIGH operational gap in the replay router (Finding 15 — `replay_suite` returned 202 but never dispatched the `ReplayRunGenerator`; the Cycle 5 GREEN-step placeholder shipped to production despite Cycles 6 + 8 landing the orchestrator-side dispatch contract), a MED classifier misclassification on explicit-creative-form prompts (Finding 14), and 6 LOW polish items (hardcoded model IDs, doc/comment drift, UI label UUID leak). All addressed at the root with regression coverage. **Final soak signals:** 0 audit-hook WARN across the entire window, 0 `WriteOnReadEngineError` raises, 0 `audit_drift` lines, P4 coverage floor exceeded on all 3 write cycles (C1=4 / C2=7 / C3=8). **Pre-release gate sheet:** 3756/3756 backend tests PASS, 1901/1901 frontend vitest tests PASS, svelte-check 1104 files / 0 errors, alembic check exit 0, ruff clean, rebase dry-run against `origin/main` succeeded across all 84 commits with zero conflicts. See [`docs/SOAK_GATES.md`](SOAK_GATES.md) for the full execution log.
