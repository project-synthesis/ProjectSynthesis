...
### v0.4.22 — Topic Probe Tier 2: save-as-suite + replay + regression alarm + 202+polling + audit-hook flip (ships 2026-05-16 — evidence-based soak-gate PASS ahead of the documented 7-day minimum per [SG-2026-05-11](SOAK_GATES.md#sg-2026-05-11--audit-hook-warnrise-flip))
...
**16 cycles (14 protocol + 2 gate) — all RED → GREEN → REFACTOR → INTEGRATE → OPERATE + V1 + V2 zero-inconsistency:**

| # | Cycle | Site | Summary |
|---|---|---|---|
| 1 | Migration + `ValidationSuite` ORM + `RunRow` FK/index | `models.py`, alembic `5576c539720f` | 11-column `validation_suite` table + 3 indexes (partial active + 2 plain). `RunRow.suite_id` FK + composite `ix_run_row_suite_id (suite_id, started_at DESC)`. Atomic forward-only migration with `inspector.has_table` / `_has_index` / `_has_fk_named` idempotency guards + reversible downgrade. 7 RED tests. |
| 2 | `ValidationSuiteService.create_from_run` + `SuiteSnapshotInputs` | `services/validation_suite_service.py` + `schemas/validation_suite.py` | Detached-ORM-safe service (Foundation P4 invariant) forks a completed `topic_probe` `RunRow` into immutable `ValidationSuite`. Frozen `SuiteSnapshotInputs` dataclass. `validation_suite_create` operation label. 11 RED tests. |
| 3 | retire/get/list/list_replays + `SuiteRetireInputs` | `services/validation_suite_service.py` extensions | 4 new methods, idempotent retire (3 NO contract on re-retire: NO write, NO event, NO trace). `validation_suite_retire` op label. Paginated envelopes (`ValidationSuiteListResponse`, `RunListResponse`). 9 RED tests. |
| 4 | `compute_regression_alarm` + 30s TTL + transition events | `services/validation_suite_service.py` + `_build_alarm_query()` module helper | `ValidationSuite × MAX(started_at) replay_run` JOIN + Python-side strict-< regression-direction filter (`latest_mean < baseline_mean - tolerance_abs` — spec §5 verbatim, no bidirectional fire). Instance-scoped 30s `TTLCache` + `_invalidate_alarm_cache()` test seam. `_prior_alarm_states` dict drives state-change-only `regression_alarm_transition` events. INTEGRATE caught GREEN's bidirectional filter bug; OPERATE cascade re-ran and verified strict-< boundary + improvement-direction non-firing. 7 RED tests. |
| 5 | `routers/suites.py` 6 endpoints + Literal extensions | `routers/suites.py`, `schemas/runs.py` Literal sites | Save-as-suite (20/min, 201), list (paginated), get, replays-list (paginated), replay (5/min, 202 + Location + Retry-After), retire (30/min). 10-code error envelope (`{detail: <code>}`). `replay_run` Literal added at 4 sites (3 schemas + 1 router query param). PATCH route absent (immutability invariant). 15 RED tests. |
| 6 | `ReplayRunGenerator` + `_aggregate.py` shared helper + orchestrator extensions | `services/generators/replay_run_generator.py` + `services/generators/_aggregate.py` + `services/run_orchestrator.py` extensions | Sequential per-prompt loop (TaskGroup rejected per inline rationale — deterministic regression detection requires sequential + provider-rate-limit-safe), full collaborator graph via `batch_pipeline.run_single_prompt(tier='internal')`. `pending.trace_id` correlation (not `.id`). 5-dimension projection + `baseline_overall` + `delta` per result. `replay_warnings: list[str]` (was `replay_repo_drift: bool` in GREEN — INTEGRATE caught + renamed per spec §2 + §5). `RunOrchestrator._persist_final` mode-keyed `replay_run_persist` op label. INTEGRATE cascade added `historical_stats` prefetch + 5-dim projection that RED tests under-pinned. 11 RED tests + INTEGRATE cascade. |
| 7 | Topic-only mode + 2nd prompt template + `ProbeContext` extensions | `services/generators/topic_probe_generator.py` branch + `services/probe_generation.py` signature + `schemas/probes.py` + `prompts/probe-agent-topic-only.md` | `grounding_mode: Literal["codebase", "topic_only"]` payload key. Topic-only path skips Phase 1 grounding, delegates Phase 2 to `generate_probe_prompts(mode='topic_only')` which inverts F1 backtick predicate to `_lacks_backtick`. `ProbeContext.repo_full_name: str | None`, `commit_sha: str | None`, `topic_only: bool = False` additive fields. Codebase-mode preserves legacy `provider.complete_parsed` path (consolidation deferred to T4 fixture migration — documented inline with Cycle 8+ forward-pointer). Router `link_repo_first` 400 gated to codebase mode only. 9 RED tests. |
| 8 | 202+polling + `dispatch_async()` + `run()` refactor | `services/run_orchestrator.py` extensions + `routers/probes.py` | `dispatch_async(*, mode, request, run_id, done_event=None)` awaits initial `_create_row()` then spawns `_run_to_completion` background task — race-free contract (immediate `GET /api/probes/{id}` finds the row). `_run_to_completion` sets `current_run_id` ContextVar inside spawned task body + `asyncio.shield(_mark_failed)` cleanup on cancel. `run()` becomes thin wrapper using dual-channel signaling (public `done_event` + private `_done_future`/`_spawned_task_holder` for exception propagation per spec §5 line 1069 unified-body preference). `Prefer: respond-async` header → 202 + Location + Retry-After; default SSE path preserved. 7 RED tests + SSE regression guard. |
| 9 | `/api/health` `regression_alarm` block | `routers/health.py` + `app.state.validation_suite_service` lifespan singleton | `HealthResponse.regression_alarm: RegressionAlarmBlock` field. Singleton on `app.state.validation_suite_service` so 30s TTL cache shares across requests. 3-path safe-default fallback (missing attribute / explicit None / compute exception) all converge on canonical empty-fleet shape — service unavailability NEVER 5xx's `/api/health`. 5 RED tests. |
| 10 | MCP tools (`synthesis_save_suite` + `synthesis_replay_suite`) — 15 → 17 | `tools/save_suite.py` + `tools/replay_suite.py` + `schemas/mcp_models.py` + `mcp_server.py` | Both `structured_output=True` with `Annotated[..., Field(...)]` boundary validation. Thin delegation to `ValidationSuiteService` + `RunOrchestrator.dispatch_async`. OPERATE caught critical functionality gap (MCP-process orchestrator was missing `replay_run` generator registration — real MCP `synthesis_replay_suite` calls would have failed `ValueError("unknown mode: replay_run")`); fix mirrors `main.py:1206-1224` byte-identically in `mcp_server.py:541-558`. 9 RED tests. |
| 11 | Topic Probe UI (4 NEW components + SeedModal 3rd tab + `@keyframes forge-spark`) | `frontend/src/lib/components/probes/{TopicProbeForm,TopicProbeProgressView,TopicProbeReportCard,TaxonomyMiniView}.svelte` + `SeedModal.svelte` 3rd tab + `shared-keyframes.css` | All h-5 buttons, p-1.5 sidebars, font-mono numerics. `forge-spark 250ms ease-out` keyframe (0%→25%→100%: scale + 3deg rotate + neon-yellow flash). Save-Suite hero button neon-purple, NEW chip neon-teal, Replay button Medium-tier neon-blue per spec §6 chromatic table. SeedModal 3rd tab "TOPIC PROBE" inherits shipped `.seed-tab` font-mono pattern. 5 spec violations caught by V1 (chromatic encoding F1+F2, density F3, svelte-check F4, hover transitions F5) all fixed inline. 18 RED tests. |
| 12 | SuitesPanel + RegressionBadge + Suites store + doc-sync | `frontend/src/lib/components/suites/{SuitesPanel,SuiteRow,SuiteDetailView,RegressionBadge}.svelte` + `frontend/src/lib/stores/suites.svelte.ts` + ActivityBar 6th tab + Navigator routing | StatusBar `RegressionBadge` mounts edge-to-edge (px-1.5 py-0 text-[10px] font-mono lower-case `12 ok` / `2 alarm`). `suitesStore` 30s health poll + `taxonomy-changed` re-fetch (also bridges Navigator's pre-existing dead-code listener). 6th `suites` ActivityBar tab + Navigator routing. Doc-sync: SKILL.md 5→7-dir scope + 22px→20px, component-patterns.md 600ms→1500ms copy-flash + 250ms forge-spark, backend/CLAUDE.md + root CLAUDE.md probe event count 7→6 + `probe_taxonomy_change` deprecation clarification, ROADMAP.md path placement + event correction. INTEGRATE cascade added full `ValidationSuiteOut` field consumption + replay-warnings strip + retired-state dimming. 14 RED tests. |
| 13 | `probes store` 202+polling abstraction | `frontend/src/lib/stores/probes.svelte.ts` (NEW) | `probesStore.runProbe(req, opts?): AsyncIterable<ProbeEvent>` — `n_prompts > 10` → auto-attaches `Prefer: respond-async` header + 5s poll cadence via `Retry-After`; `n_prompts ≤ 10` → SSE path. AbortSignal cleanly cancels both (no leaked timers). Consumer reads shape-identical events regardless of transport. 5 client-side observability event names (`probe_polling_heartbeat` etc.) emitted in poll path under documented spec §9 server-canonical 6-event boundary. 7 RED tests. |
| 14 | Brand + a11y audit (GATE — V1 brand + V2 a11y only) | grep audits across 7-dir scope + axe-compatible static checks | Brand: 0 hits for box-shadow blur/spread, text-shadow, drop-shadow, banned vocab (word-boundary regex permits hyphenated canon: `copy-flash`, `forge-spark`, `useCopyFlash`). A11y: 2 findings fixed inline — `TopicProbeForm.svelte` `:focus` → `:focus-visible` (canonical 1px cyan outline inheritance restored); `RegressionBadge.svelte` gained `role="status"` + `aria-live="polite"` for screen-reader state-change announcement on nominal→firing transition. |
| 15 | Audit-hook WARN→RAISE flip (≥2026-05-18 post-soak gate) | `backend/app/config.py` | `WRITE_QUEUE_AUDIT_HOOK_RAISE: bool = Field(default=True, ...)` (was `False`). Field description documents kill-switch path: `WRITE_QUEUE_AUDIT_HOOK_RAISE=false` env var reverts to v0.4.21 WARN-only behavior. Existing audit-hook logic at `backend/app/database.py:395-397` unchanged. Manual soak grep gate (`grep -E "audit_drift\|read-engine audit:" data/backend.log | tail -100`) deferred to release operator (today 2026-05-12, gate ≥2026-05-18). New `tests/test_audit_hook_full_t2_pipeline.py` (3 tests `@pytest.mark.integration`): default-is-RAISE, zero-WARN-under-full-T2-pipeline, kill-switch env var reverts. Existing P4 `test_audit_hook_emits_zero_warn_under_full_pipeline` unchanged + still PASSing. |
| 16 | E2E + soak grep + pre-release checklist (GATE) | 14-item pre-release checklist | E2E coverage mapping (9 steps from spec §13 success criterion 4) all mapped to existing passing tests; manual scoring-degradation gates (steps 5+8) deferred to release operator. Pre-release checklist 14 items: brand audit PASS, a11y audit PASS, alembic check exit 0, backward compat (no Prefer header → SSE preserved), 2 doc-drift findings fixed (ROADMAP.md `probe_taxonomy_change` event reference + spec §13.3 frontend coverage threshold 80%→78% to match shipped `vite.config.ts` post-v0.4.19 baseline). Manual operator gates (soak grep, `./init.sh restart` smoke, CHANGELOG/SHIPPED authoring) deferred to release task. |
| post | Full-stack engineer pre-PR audit + UI wiring fixes | `SeedModal.svelte` + `SuiteDetailView.svelte` | Full-stack engineer subagent audit caught **PR NO-GO blocker**: SeedModal 3rd tab was a non-functional stub (Start Seed button dispatched malformed `/api/seed` payloads for `topic_probe` mode) + 4 NEW Topic Probe UI components were orphaned (no UI mount point). Fix: SeedModal 3rd tab mounts full 3-state inline workflow (`idle` → `TopicProbeForm`, `running` → `TopicProbeProgressView`, `completed` → `TopicProbeReportCard`); modal-footer Start Seed button hidden in `topic_probe` mode (TopicProbeForm owns its own submit through `probesStore.runProbe()`); SuiteDetailView gains Replay (Medium-tier neon-blue 200ms transition) + Retire (destructive, DestructiveConfirmModal-gated, hidden when retired) buttons. Topic Probe lifecycle now fully reachable from workbench (was API/CLI/MCP-only). All 1901 frontend tests + 150 backend Cycle 1-16 tests pass post-fix. |

**Backward compat preserved across all four surfaces:** `POST /api/probes` without `Prefer:` header returns SSE response identical to v0.4.21; T1 probes work end-to-end with zero behavior change; existing seed agent flows unchanged; `GET /api/runs?mode=...` accepts the new `replay_run` Literal value (Pydantic schemas extended without breaking existing `topic_probe`/`seed_agent` callers); `synthesis_probe` + `synthesis_seed` MCP tools unchanged (only 2 new tools added, 15→17). Audit-hook flip preserves all v0.4.21 behavior for callers that opt out via `WRITE_QUEUE_AUDIT_HOOK_RAISE=false`.

**Stats:** 54 commits on `feature/probe-tier-2` (worktree branch `worktree-feature+probe-tier-2`). Backend regression: ~150 Cycle 1-16 tests + ~3650 broader passing; frontend: 1901 passed / 0 failed (130 files); ruff + svelte-check + alembic check all clean. `WriteQueue` operation labels: 3 new (`validation_suite_create`, `validation_suite_retire`, `replay_run_persist`). Event bus: 4 new events (`validation_suite_created`, `validation_suite_retired`, `regression_alarm_transition`, `probe_warning`). MCP tools: 15 → 17 (`synthesis_save_suite`, `synthesis_replay_suite`).

**Foundation phase status:** P1 + P2 Path A + P3 + P4 complete. T2 ships at v0.4.22 on the unified `RunRow` substrate. T3 (cross-tier composition: probe → seed-agent promotion, release.sh CI hook, drill-into-cluster) lines up for v0.4.23 + T4 (final UI consolidation — SeedModal 3 → 1 tab) for v0.4.24 — both build natively on the substrate with zero retroactive migration.

**Soak gate execution (SG-2026-05-11, 2026-05-12 → 2026-05-16, closed Day 5 ahead of the 7-day minimum):** **21 findings** surfaced + addressed across the soak window — 5 HIGH + 6 MED + 8 LOW + 2 architectural improvements. Day 1 caught the v0.4.22 audit-hook RAISE-flip blockers (`passthrough_save` / `update_optimization` read-session mutations + 5 missing `app.tools._shared` singletons in lifespan + 3 empty-commit-closure endpoints + refinement rollback FK seeding). Day 4 supplementary surfaced a HIGH test-infrastructure dead-coverage gap on the audit-hook fixture (Finding 7), Finding 14's classifier creative-form-marker bypass, Finding 15's replay router dispatch gap (Cycle 5 placeholder shipped to production despite Cycles 6+8 landing the orchestrator-side contract), and 6 LOW polish items. Day 5 attempted a real-content suite end-to-end and surfaced 5 additional bugs: Finding 16 (HIGH — TopicProbeGenerator stub never wired to `batch_pipeline.run_single_prompt`, blocked every Topic Probe T1 run since v0.4.18), Finding 17 (MED — `GET /api/probes/{id}` 500 on legacy `raw_prompt` rows), Finding 18 (LOW — rate-limit `provider="unknown"` sentinel corrupting routing state), Finding 19a (LOW — wrong attribute name in projection dict showing `score=None`), Finding 20 (MED — snapshot builder only read `raw_prompt` so topic-probe suites had empty strings). Finding 19b (`bulk_persist` wiring) deferred to v0.4.23 — the surface ships intact because save-as-suite reads `RunRow.prompt_results` directly. All addressed at the root with 18 new regression tests. **Final soak signals:** 0 audit-hook WARN across the entire window, 0 `WriteOnReadEngineError` raises, 0 `audit_drift` lines, P4 coverage floor exceeded on all 3 write cycles (C1=4 / C2=7 / C3=8). **Pre-release gate sheet:** 3774/3774 backend tests PASS, 1901/1901 frontend vitest tests PASS, svelte-check 1104 files / 0 errors, alembic check exit 0, ruff clean, rebase dry-run against `origin/main` succeeded across all 84 commits with zero conflicts. See [`docs/SOAK_GATES.md`](SOAK_GATES.md) for the full execution log.

**Spec:** `docs/superpowers/specs/2026-05-11-topic-probe-tier-2-design.md` (14 review rounds, APPROVED-ZERO at commit `9b843791`). **Plan:** `docs/superpowers/plans/2026-05-11-topic-probe-tier-2.md` (~1150 LOC, 16 cycles, V2 APPROVED-ZERO at commit `91655a25`). **Migration:** `5576c539720f_validation_suite_topic_probe_t2.py` (atomic forward-only with idempotency guards + reversible downgrade).
...
**Acknowledged exception — cold path:** taxonomy cold-path full-refit retained on `WriterLockedAsyncSession` + `cold_path_mode` audit-hook bypass. Refit's transaction span (multi-second commits across thousands of cluster rows) does not fit the queue's per-task timeout model. v0.4.15 chunks each refit phase into smaller `submit()` calls with `await asyncio.sleep(0)` between.
...
**Spec:** `docs/specs/sqlite-writer-queue-2026-05-02.md` (v6 APPROVED). **Plan:** `docs/superpowers/plans/2026-05-02-sqlite-writer-queue.md` (v3 APPROVED). **Branch:** `release/v0.4.13` (76 commits, all pushed). **Stats:** 76 commits, 11 TDD cycles, ~11000 LOC. **Topic Probe Tier 2** (originally targeted for v0.4.13) deferred — first re-targeted to v0.4.14, then to v0.4.15 once v0.4.14 was reallocated to the SQLite migration finalization. Each release was scoped on the architectural fix that all subsequent Probe tiers depend on.
