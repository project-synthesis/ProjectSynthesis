...
| Q6 | Rollout | **Two PRs.** PR1 = dark substrate (RunRow + RunOrchestrator + generators + tests, no router wiring). PR2 = wire shims atomically + frontend `run_id` filter additive. Both delivered under PR #70. T4 (v0.4.24) does the unified UI. |
...
After Foundation P3 + P4 + Probe T2-T4 ship (target window: v0.4.18-v0.4.24), the architecture is ready for a v0.5.0 major. Candidate themes (none yet committed):
...
**Status:** **Tier 1 SHIPPED** (v0.4.12, 2026-04-29). Tier 2 / Tier 3 / Tier 4 remain **Planned**, sequenced AFTER the foundation phase + v0.4.19 stability slot + v0.4.20 Pattern-Graph-canon-and-auth-hotfix slot: T2=v0.4.22, T3=v0.4.23, T4=v0.4.24. **Substrate unification — originally scoped into T4 — SHIPPED early at v0.4.18 (Foundation P3, 2026-05-09)**, so T2/T3/T4 features now build natively on the unified `RunRow` substrate with zero retroactive migration debt. Tier 2 was bumped repeatedly through public releases — v0.4.13 → v0.4.14 → v0.4.15 → v0.4.20 → v0.4.21 → v0.4.22 — as architectural fixes (SQLite contention, SQLite migration finalization, HistoryPanel pagination, foundation phase, v0.4.19 stability slot, v0.4.20 Pattern Graph canon + auth hotfix slot) shipped first; see SHIPPED.md + the "Foundation phase" entry above for the complete rationale.
