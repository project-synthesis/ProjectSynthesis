...
**Snapshot:** v0.4.27 SHIPPED 2026-05-19 — Pattern Graph architecture hardening program **COMPLETE** (all 6 sub-projects A→F shipped across v0.4.23-v0.4.27). **Probe Tier 3 decomposition in progress** (2026-05-19): originally bundled v0.4.28; now decomposed into v0.4.28 (Finding 19b + T3.1 release.sh CI hook), v0.4.29 (T3.2 promotion + T3.5 cross-tier composition), v0.4.30 (T3.3 drill-into-cluster), v0.4.31 (Probe Tier 4). **Finding 19b MERGED to main 2026-05-19 (PR #89)**; held on main for T3.1 bundle. **T3.1 currently in brainstorming**. Earlier highlights still in scope: v0.4.22 SHIPPED 2026-05-16 — Probe Tier 2 + audit-hook WARN→RAISE flip + 21 soak findings addressed pre-ship. v0.4.20 SHIPPED 2026-05-10 — Pattern Graph 3D canon F1–F19 retrieval + F19 plasma envelopement burst + GitHub Device Flow auth hotfix chain. v0.4.18 SHIPPED 2026-05-09 — Foundation P3 substrate unification.

**Pattern Graph architecture hardening — COMPLETE 2026-05-19** (6/6 sub-projects shipped across PRs #83-#88). 6-sub-project refactor of `frontend/src/lib/components/taxonomy/` that addressed `SemanticTopology.svelte`'s pre-program 2834-line monolith + 228-commit churn since 2026-02-01. Release timeline: **v0.4.23** (A Lifecycle Hardening + B AnimationCoordinator combined, 2026-05-17) → **v0.4.24** (C ImpactCoordinator, 2026-05-17) → **v0.4.25** (D SceneBuilder Extraction, 2026-05-18; -766 LOC) → **v0.4.26** (E SelectionController + FlashController, 2026-05-19) → **v0.4.27** (F Audit-as-Code, 2026-05-19; 53 audit-canon tests + meta-test gate). `SemanticTopology.svelte` final LOC: 1837 (down from 2834 pre-program). Audit checklist now mechanically enforced at CI time. Next: **Probe Tier 3 (v0.4.28)** + **Tier 4 (v0.4.29)** Planned. See "Pattern Graph architecture hardening" entry under Immediate for the full breakdown (A→F, dependency-ordered, all SHIPPED).
...
- ~~**P4 Long-handler restructures**~~ — **SHIPPED v0.4.21 (2026-05-11)**: 3 long-running MCP tool handlers (`tools/save_result.py`, `tools/refine.py`, `tools/optimize.py` internal tier) restructured so LLM calls live outside DB sessions and persistence routes through `WriteQueue.submit()`. Closes the final v0.4.13 SQLite migration tail. 3 TDD cycles (one per handler), integration Test 13 (`test_audit_hook_emits_zero_warn_under_full_pipeline`) PASS — v0.4.22 audit-hook WARN→RAISE flip precondition MET.
...
### Pattern Graph architecture hardening — 6-sub-project refactor program (SHIPPED COMPLETE)
**Status (2026-05-19):** **COMPLETE — all 6 sub-projects A→F SHIPPED across v0.4.23-v0.4.27** via PRs #83-#88. Program ran from 2026-05-17 through 2026-05-19. Final deliverable: Sub-project F (Audit-as-Code) introduces `audit-canon.test.ts` + `_canon-coverage.test.ts` meta-test that mechanically enforces F1-F19 canon at CI time. Adding a new canonical feature now requires updating BOTH the canon doc AND `audit-canon.test.ts` in the same commit — the meta-test catches the omission. `SemanticTopology.svelte` reduced 2834 → 1837 LOC (-997 LOC across the program). Per-sub-project breakdown in the dependency-chain table below.
**Specs:** One spec + plan per sub-project under `docs/superpowers/specs/` and `docs/superpowers/plans/`. First spec landed at `docs/superpowers/specs/2026-05-16-lifecycle-hardening-design.md` + plan at `docs/superpowers/plans/2026-05-16-lifecycle-hardening-plan.md`. B–F brainstorming starts after A merges; each follows the same gated workflow (brainstorm → spec → independent reviewer loop → plan → independent reviewer loop → subagent-driven execution → spec-compliance + code-quality reviewer gates).
**Release allocation:** one version per sub-project. See "Release allocation" table at the top of this file (under "Foundation phase") — Pattern Graph hardening occupies v0.4.23–v0.4.27, pushing Probe Tier 3 to v0.4.28 and Tier 4 to v0.4.29.
...
| # | Sub-project | Target version | Status | Depends on | Notes |
|---|---|---|---|---|---|
| **A + B** | **Sub-projects A + B (Lifecycle Hardening + Animation Coordinator)** — combined release. (A) `userData.persistent = true` convention + extracted `cleanupScene` helper; fixes lights/dust/envelope orphan-group class of bugs; canon F11/F12 (lights, shadows) compliance restored; replaces ~30 lines of manual save/restore boilerplate. (B) Single per-frame tick with explicit ordered phases (impact → physics → breathing → ambient pulse → camera); eliminates per-callback registration-order coupling; phase ordering contract documented in `references/3d-visualization.md`. | **v0.4.23** | **SHIPPED 2026-05-17** (PR #83 A; PR #84 B) | nothing | Both shipped within a single session window 2026-05-16 → 2026-05-17 and bundled as a single combined release. New files `scene-cleanup.ts` (A) + `AnimationCoordinator.ts` (B) + tests; constructor edits to `BeamPool` / `EnvelopePool` / `TopologyLabels` / `TopologyRenderer` (A); ~10 separate `addAnimationCallback` registrations consolidate into one ordered pipeline (B). |
| **C** | **Impact Coordinator** — single function chains beam → envelope → flash → physics → idle-pulse. Eliminates the 4 duplicate trigger sites (entrance burst, post-growth burst, optimization event, click selection). Canon F7 causal-ordering invariant enforced at source level by routing every site through the same coordinator. | **v0.4.24** | **SHIPPED 2026-05-17** | A, B | New file `ImpactCoordinator.ts` + `TRIGGER_PRESETS` table + 22 unit tests + 5 integration tests + 12 source-grep assertions. Routes all 4 trigger sites through `impactCoordinator.fire({trigger, node, group})`. T3.4 idle ambient pulse extracted to coordinator (framerate-independent). `_selectionEngulfed` ownership migrated to coordinator (exposed via `isEngulfed(id)`). 515/515 taxonomy tests green; svelte-check 0/0. |
| **D** | **Scene Builder Extraction** — split the 774-line `rebuildScene` into 5 single-responsibility builder modules: `ClusterBuilder`, `DomainBuilder`, `EdgeBuilder` (hierarchical + similarity + injection), `RingBuilder` (readiness + template), `DustBuilder`. Each unit-testable in isolation via a shared `BuilderContext` interface. Diff-based rebuild explicitly OUT OF SCOPE — captured as Exploring entry "Diff-based scene rebuild" below for post-D consideration. | **v0.4.25** | **SHIPPED 2026-05-18** | A, B, C | 6 cycles × 5 phases + 2 review gates. New directory `frontend/src/lib/components/taxonomy/builders/` with `BuilderContext.ts` + `SceneBuilder.ts` interface + 5 builder classes + per-builder unit tests + 6 integration test cases. 16 source-grep contract assertions in `cleanup-contract.test.ts` pin the migration structurally. `SemanticTopology.svelte` drops from 2794 → 2028 LOC (-766 LOC of inline construction); `rebuildScene` body drops from 714 → 144 LOC (under the 150 LOC ceiling). Build order fixed: cluster → domain → edge → ring → dust. `ImpactCoordinator` physics handler reads `ctx.clusterShaderMaterials` instead of `group.children[1]` index walk (locks INT-6 architecture improvement). 618 taxonomy tests green, 8 deferred (T2.2 transitions + readiness ring tier-tween + size-drift-rebuild — scoped out of RingBuilder as visual polish for follow-on). |
| **E** | **Selection State Machine** — SC owns highlight/engulfed/flash lifecycle; F16 hack replaced by deterministic afterRebuild. Wraps the 5 parallel selection-related module refs (`_flashStates`, `_selectionEngulfed`, `_highlightedId`, `_prevSelectedId`, `focusedNodeId`) into one `SelectionController` with explicit 5-state diagram + extracts `FlashController` from inline `flashEmissive` + `_flashStates`. The canon F16 highlight-survival hack is replaced by `selectionController.afterRebuild()` invoked deterministically at the end of `rebuildScene`. | **v0.4.26** | **SHIPPED v0.4.26** | B, C (depends on impact + animation pipelines being stable) | 49 new tests (14 FC + 30 SC unit + 5 SC integration) + 18 source-grep assertions. `ImpactCoordinator` shrinks: engulfed-set + `_pulseTime` + `_tick` + `isEngulfed/clearEngulfed` + `SELECTION_EMISSIVE_FLOOR` export removed. |
| **F** | **Audit-as-Code** — `frontend/src/lib/components/taxonomy/audit-canon.test.ts` (53 tests) + `_canon-coverage.test.ts` meta-test enforce every canon checklist line via source-grep + runtime contract tests. `_canon-coverage.test.ts` parses the canon markdown and verifies every `- [ ] **F<N>**: ...` line maps to a test. Failing audit = failing CI. Replaces manual audit-pass. | **v0.4.27** | **SHIPPED v0.4.27** | A, B, C, D, E (canon-faithful state preserved) | 53 new audit-canon tests + 1 meta-test. Final closure of the Pattern Graph hardening program. |
...
### Foundation phase — zero-tech-debt prep for Probe Tier 2-4 (v0.4.16-v0.4.21)
**Status:** P1 + P2 Path A + P3 + P4 shipped (v0.4.16, v0.4.17, v0.4.18, v0.4.21). P2 Path B remaining (deferred indefinitely). Updated 2026-05-11.
...
| **P4** | **Long-handler restructures** (separate read/process/write phases in `tools/refine.py:50,:156`, `tools/save_result.py:85`, `tools/optimize.py:198` so the LLM call lives outside any session and persistence boundaries route through the queue) | **SHIPPED v0.4.21** (2026-05-11) | nothing (probe-independent but bundled in foundation envelope) | Closes the final SQLite migration tail. 3 TDD cycles (one per handler), integration Test 13 PASS — v0.4.22 audit-hook WARN→RAISE flip precondition MET. See dedicated "Foundation P4 — Long-handler restructures" section below. |

**Release allocation (revised 2026-05-10 third pass — v0.4.20 Pattern Graph + auth hotfix slot + Probe shift):**
...
- **v0.4.19** (2026-05-09) — Stability/UX patch release: brand-driven motion personality + 4 SSE reactivity fixes (4-gap close, cold-boot flicker, tier-trigger consolidation) + write-queue `worker_alive` correctness + frontend coverage 80% baseline — **SHIPPED**
- **v0.4.20** (2026-05-10) — Pattern Graph 3D canon F1–F19 + GitHub Device Flow auth hotfix chain. Pattern Graph: atmospheric/cinematic/tactile canon retrieval (F1–F18 work originally walked back, restored alongside complementary improvements) + new F19 plasma envelopement burst at beam impact (`EnvelopePool` + `EnvelopeShader` + `BeamConfig.onImpact` callback synchronizing every impact reaction to actual beam arrival) + click-zoom click-flow stability (LOD-rebuild during focus animation + galaxy formation re-firing on every reactive `$effect` re-run, both fixed). GitHub auth: `_classify_poll_error()` + frontend bounded-retry counter close the silent infinite-poll trap from upstream HTML 5xx; `app.tools._shared.set_write_queue` wired into FastAPI lifespan closes the latent token-persistence 500. — **SHIPPED**
- **v0.4.21** (2026-05-11) — Foundation P4 (long-handler restructures: refine/save_result/optimize handlers) — **SHIPPED**
- **v0.4.22** — Probe Tier 2 (save-as-suite + replay + UI + regression alarm) + audit-hook WARN→RAISE flip — **SHIPPED 2026-05-16** (evidence-based soak-gate PASS on Day 5 ahead of the documented ≥2026-05-18 minimum per SG-2026-05-11 decision-matrix row 2; 16 cycles complete, **21 soak findings** caught + addressed pre-ship (5 HIGH + 6 MED + 8 LOW + 2 architectural), all 5 CI checks green on `473c5baa`, rebase against `origin/main` verified conflict-free across 96 commits, 3776/3776 backend tests PASS)
- **v0.4.23** — Pattern Graph architecture hardening **Sub-projects A + B SHIPPED 2026-05-17 (combined release)**. (A) **Lifecycle Hardening** (`userData.persistent = true` convention + `cleanupScene` helper; fixes 3-light orphan + restores canon F11/F12; closes per-domain `PointsMaterial` leak) — spec APPROVED revision 8 (8 reviewer rounds); plan APPROVED revision 2 (1 reviewer round); 12-dispatch subagent-driven execution per `feedback_tdd_protocol.md` strict 5-phase TDD; merged via PR #83. (B) **Animation Coordinator** (single ordered per-frame tick with documented phases: impact → physics → breathing → ambient → camera; eliminates implicit registration-order coupling) — merged via PR #84. Both completed within a single session window (2026-05-16 → 2026-05-17) and bundled as one combined release rather than separate minors.
- **v0.4.24** — Pattern Graph hardening **Sub-project C — Impact Coordinator** (single function chains beam → envelope → flash → physics → idle-pulse; eliminates 4 duplicate trigger sites; F7 causal-ordering enforced at source level) — **SHIPPED 2026-05-17**. New `ImpactCoordinator.ts` + `TRIGGER_PRESETS` table consolidates entrance/post-growth/optimization/click trigger sites behind `impactCoordinator.fire({trigger, node, group})`. T3.4 idle ambient pulse extracted to coordinator (framerate-independent). 22 unit + 5 integration + 12 source-grep tests pin the contract. 515/515 taxonomy tests green; svelte-check 0/0.
- **v0.4.25** — Pattern Graph hardening **Sub-project D — Scene Builder Extraction** (split 774-line `rebuildScene` into 5 builder modules: `ClusterBuilder`, `DomainBuilder`, `EdgeBuilder`, `RingBuilder`, `DustBuilder` under `frontend/src/lib/components/taxonomy/builders/`; `SemanticTopology.svelte` drops from 2794 → 2028 LOC, `rebuildScene` body from 714 → 144 LOC) — **SHIPPED 2026-05-18**. Largest of the 6 sub-projects — ~32 dispatches across 6 cycles + 2 review gates. New `BuilderContext` interface threads shared per-rebuild state through every `build(data, scene, ctx)` call; `ImpactCoordinator` physics handler reads `ctx.clusterShaderMaterials` instead of walking `group.children[1]` (locks the architecture improvement at source level via INT-6).
- **v0.4.26** — Pattern Graph hardening **Sub-project E — Selection State Machine** (consolidate 5 parallel selection refs into one `SelectionController` with explicit 5-state diagram + extract `FlashController` from inline `flashEmissive` + `_flashStates`; SC owns highlight/engulfed/flash lifecycle; canon F16 highlight-survival hack replaced by deterministic `selectionController.afterRebuild()` invoked at the end of `rebuildScene`) — **SHIPPED v0.4.26**. 49 new tests (14 FC + 30 SC unit + 5 SC integration) + 18 new source-grep assertions; M3 migration sweep across 13 `SemanticTopology.test.ts` `it()` blocks + 5 `cleanup-contract.test.ts` `it()` blocks. `ImpactCoordinator` shrinks: `_selectionEngulfed`, `_pulseTime`, `_tick`, `isEngulfed`, `clearEngulfed`, `SELECTION_EMISSIVE_FLOOR` export, and the `getSelectedId` / `isFlashActive` deps deleted.
- **v0.4.27** — Pattern Graph hardening **Sub-project F — Audit-as-Code** — **SHIPPED v0.4.27**. `audit-canon.test.ts` (53 tests, one per canon checklist line) + `_canon-coverage.test.ts` meta-test enforce the F1-F19 audit gate at CI time. Final closure of the Pattern Graph hardening program — all 6 sub-projects (A Persistence, B AnimationCoordinator, C ImpactCoordinator, D SceneBuilder, E SelectionController, F Audit-as-Code) shipped.
...
- **TBD** — RunsPanel: multi-select + bulk actions on RunRows. Bulk delete / bulk rename / bulk export. v0.4.31 ships per-row inline detail only. No version target; folds in with the per-row rename + delete entry below.
- **TBD** — RunsPanel: per-row rename + delete actions. Matches existing HistoryPanel per-row pattern. Could fold into a v0.4.32 sweep with bulk actions.
- **TBD** — RunsPanel: text search input across topic / agent / suite-label fields. v0.4.31 filtering is chip-only (mode + status). Could fold into a UX-pass cycle.
- **TBD** — RunsPanel: periodic auto-refresh for in-progress runs. Poll `GET /api/runs?status=running` every N seconds OR repurpose existing `seed-batch-progress` SSE channel. Out of v0.4.31 scope.
- **TBD** — RunsPanel: cross-mode bulk export (JSON / CSV). YAGNI for v0.4.31; could ship in a future ops/observability cycle.
- **TBD** — RunsPanel: persisted filter preferences via `localStorage`. v0.4.31 filters reset on tab close. Low-priority polish.
- **TBD** — RunsPanel: deep-linking (`/runs/[run_id]` route). Browser-back-friendly URLs. v0.4.31 uses inline expand only.
- **TBD** — RunsPanel: re-run / fork action on RunRow. "Run this again with these params" affordance. Out of v0.4.31 scope.
- **TBD** — RunsPanel: live replay-in-progress polling. `mode='replay_run' && aggregate=null` rows currently show "Replay in progress" static placeholder. SSE infrastructure exists; just needs wiring.
- **TBD** — Runs vs History unification. v0.4.31 ships Runs + History as separate sidebar tabs (different grains: RunRow batches vs single Optimizations). Future cycle may explore unified surface if operator feedback warrants.
- **TBD** — RunsPanel: bulk-rename via modal (single dialog with field per selected row OR shared display_name template). v0.4.32 ships per-row rename only.
- **TBD** — RunsPanel: CSV export format (`?format=csv` query param on bulk-export). v0.4.32 ships JSON only.
- **TBD** — RunsPanel: bulk-export pagination/streaming beyond 200-id limit. v0.4.32 caps at 200.
...
- **TBD** — RunRow result-JSON retention/compaction policy (v0.4.37 N3). Replay output capture adds ≈600 KB worst case per replay (25 prompts × 24 KB); caps + `output_truncated` flag ship in v0.4.37, retention does not. Revisit if replay volume grows.
- **TBD** — Audit-log coverage beyond optimizations (v0.4.37 N4): suite retire/rebuild, cluster dissolution, template retire are still unattributable. v0.4.37 scoped `audit_log` writes to user-initiated optimization deletes only.
...
**v0.4.20 Pattern Graph + auth hotfix slot + third P4 re-allocation (2026-05-10):** Between v0.4.19 shipping and the planned v0.4.20 P4 cycle starting, the Pattern Graph 3D canon retrieval work merged on `feature/restore-pattern-graph-canon` (28 commits — atmospheric/cinematic/tactile F1–F18 retrieval that had been walked back under an over-restrictive interpretation, plus the new F19 plasma envelopement burst at beam impact, plus two click-zoom click-flow stability fixes). The PR went through 8 code-review rounds reaching APPROVED-ZERO before rebase-merge into main. Two GitHub Device Flow auth hotfixes followed (silent infinite-poll trap on upstream HTML 5xx + token-persistence 500 from a latent `app.tools._shared` singleton wiring gap). Cutting these as v0.4.20 preserves a clean release boundary for the visual + integration-stability work and keeps P4's diff focused on its own scope. P4 slips one more minor to v0.4.21; Probe Tier 2-4 shift another minor (T2=v0.4.22, T3=v0.4.23, T4=v0.4.24).

**v0.4.23–v0.4.27 Pattern Graph architecture hardening slot + Probe T3-T4 re-allocation (2026-05-16; revised 2026-05-17 to bundle A + B):** Post-v0.4.22 (Probe T2) shipping, a 6-sub-project Pattern Graph architecture hardening program was scoped to address the architectural debt in `frontend/src/lib/components/taxonomy/` — `SemanticTopology.svelte` at 2834 lines with 23 functions / 8 `$effect` blocks / 8 parallel state maps; 228 commits since 2026-02-01 with 5 sequential engulfment-burst fixes and 5+ race-condition workarounds; the just-discovered lights orphan from `rebuildScene`'s indiscriminate wipe loop (commit `025b0048` shipped the envelope-pool fix; full program targets the class of bug). Sub-project A spec APPROVED at revision 8 + plan APPROVED at revision 2 as of 2026-05-16, with the gated workflow (brainstorm → spec → independent-reviewer loop → plan → independent-reviewer loop → 5-phase TDD subagent-driven execution → spec-compliance + code-quality reviewer gates) producing 36 findings across 8 spec-review rounds before convergence. **Sub-projects A and B both completed and merged within a single session window (2026-05-16 → 2026-05-17) and shipped as one combined v0.4.23 release** rather than separate minors; C–F continue to ship one per minor (v0.4.24 → v0.4.27) to preserve TDD provenance and provide clean rollback points. Sub-projects C–F have spec seeds at `docs/superpowers/specs/2026-05-16-pattern-graph-hardening-program-context.md` enabling fresh-session resume. Probe Tier 3 shifts from v0.4.23 to v0.4.28; Tier 4 from v0.4.23 to v0.4.29. The shift adds ~4 minor versions of architectural depth before T3 picks up Finding 19b restoration; the rationale is that T3-T4 layer significant new UI surfaces onto the taxonomy directory, and shipping them on the already-strained `SemanticTopology.svelte` monolith would cost more in retroactive integration than the architectural refactor costs upfront. The Pattern Graph hardening program's program-level acceptance criteria (file < 800 lines, F1–F19 audit-as-code, ordered tick, single impact path, zero indiscriminate wipes, single selection owner) are documented in the Immediate-section entry.

---

### Foundation P3 — Substrate unification (v0.4.18)
**Status:** **SHIPPED v0.4.18 (2026-05-09)** — PR #70, 95+ commits on `release/v0.4.18`. Implementation followed plan APPROVED-ZERO-V5 with strict 7-dispatch TDD per cycle. Detailed shipping notes in [`SHIPPED.md`](SHIPPED.md#v0418--2026-05-09).

**What shipped:** A unified `RunRow` ORM (18 columns, 4 indexes, 4-value status `running/completed/failed/partial`) + `RunOrchestrator` service that dispatches to pluggable generators conforming to a `RunGenerator` Protocol (`TopicProbeGenerator` for the agentic-from-topic-and-codebase path, `SeedAgentGenerator` for template-driven seed runs). `ProbeRun` ORM class deleted; `ProbeService` class deleted; `probe_service.py` 2270 → 37 LOC re-export shim. The seed surface gained row-state persistence for the first time. All current REST + MCP surfaces (`/api/probes`, `/api/seed`, `synthesis_probe`, `synthesis_seed`) kept their endpoints and response shapes via backward-compat shims that translate to/from the unified substrate. New unified endpoints `GET /api/runs` + `GET /api/runs/{run_id}` (paginated, mode/status/project_id filters) + `GET /api/seed` + `GET /api/seed/{run_id}` (additive). New `event_bus.subscribe_for_run(run_id)` filtered async iterator with 500ms ring-buffer replay coexists with the existing `EventBus` without modifying `publish()`.

**Pre-shipping asymmetry (snapshot from brainstorm, verified 2026-05-06 against `main` @ v0.4.18-dev — preserved here for context):**

| Layer | Probe (pre-P3) | Seed (pre-P3) |
|---|---|---|
| Persistence model | `ProbeRun` — 17 columns (`models.py:570`, migration `ec86c86ba298`) | **none** — no `SeedRun` model has ever existed; only persisted artifact of a seed run was the resulting `Optimization` rows tagged `source="batch_seed"` |
...
| Frontend | zero code — no `probe.ts` API client, no `Probe*` components; the REST + MCP surfaces had no SvelteKit consumer | `SeedModal.svelte` rendered live progress + final summary in-modal; no history component |

P3 was therefore not "collapse two models into one" — it was "introduce run-state to the seed surface AND reshape probe run-state into a generic `RunRow` substrate at the same time, without regressing any existing surface contract." Both halves landed in v0.4.18.

**Why this mattered:**

1. **T2 save-as-suite + replay** keys off `RunRow.id`. Without P3, the seed surface had no row-state to attach a save-as-suite operation to — save-as-suite would have shipped probe-only or seed would have grown ad-hoc persistence.
2. **T3 probe→seed-agent promotion** becomes a `RunRow.mode` flip plus a metadata write. Without P3, promotion had nothing on the seed side to read from.
3. **T4 final UI consolidation** (SeedModal becomes one tab with two modes) now ships natively with one history surface. P3 enables that history surface to be built once on top of `RunRow` rather than twice.
4. **Lifecycle parity, end of helper drift** — pre-P3 only `_set_probe_status` existed; there was no seed-side equivalent. `RunOrchestrator` owns the persistence helpers once and both modes inherit identical status/error/GC behavior.

**Implementation summary (95+ commits across `release/v0.4.18`, 16 cycles + 2 prework cycles + 1 follow-up cycle):**

```
backend/app/models.py
  RunRow                      → 18 columns: id, mode ∈ {seed_agent, topic_probe}, status,
                                started_at, completed_at, error, project_id FK,
                                repo_full_name, topic (nullable, NULL for seed),
                                intent_hint (nullable), prompts_generated,
                                prompt_results JSON, aggregate JSON, taxonomy_delta JSON,
                                final_report TEXT, suite_id, topic_probe_meta JSON,
                                seed_agent_meta JSON. 4 indexes (mode+started_at,
                                status+started_at, project_id, topic). 4-value status
                                (running / completed / failed / partial).
  ProbeRun                    → DELETED in same Alembic migration after backfill.

backend/app/services/
  run_orchestrator.py         → RunOrchestrator: only legitimate writer of RunRow rows.
                                Routes all writes through WriteQueue.submit(). Owns
                                current_run_id ContextVar lifecycle, cancellation
                                handling under asyncio.shield(), exception capture
                                with 2000-char truncation, 4-status terminal
                                transitions.
  generators/
    base.py                   → RunGenerator Protocol: `async def run(request, *, run_id)
                                -> GeneratorResult`. Progress events publish to
                                event_bus directly with run_id in payload.
    topic_probe_generator.py  → ~760 LOC, refactored from ProbeService._run_impl;
                                preserves all 5 phases + 8 SSE event types byte-for-byte
    ...
    seed_agent_generator.py   → ~330 LOC, refactored from tools/seed.py:handle_seed.
                                Preserves early-failure HTTP 200 + 'partial'
                                classification + decision events on Channel 2
                                taxonomy_event_logger. seed_batch_progress payload
                                gains run_id.
  probe_service.py            → 2270 → 37 LOC re-export shim (preserves the
                                `current_probe_id` import path; ProbeService class
                                deleted, ProbeRun ORM class deleted).

backend/app/routers/
  runs.py                     → NEW — GET /api/runs (paginated list, mode/status/
                                project_id filters, ordered started_at desc),
                                GET /api/runs/{run_id}.
  probes.py                   → backward-compat shim — POST returns race-free SSE via
                                subscribe-before-dispatch (caller mints run_id);
                                ValidationError → 400 invalid_request.
  seed.py                     → backward-compat shim — POST /api/seed stays
                                SYNCHRONOUS; SeedOutput gains additive run_id;
                                NEW GET /api/seed + GET /api/seed/{run_id}.

MCP tools/
  synthesis_probe             → response shape unchanged; backend dispatches via
                                RunOrchestrator. bus→ctx progress bridge restored
                                AC-C6-3 (filtered subscribe_for_run forwards
                                probe_prompt_completed → ctx.report_progress).
  synthesis_seed              → SeedOutput unchanged + additive run_id field;
                                tools/seed.py 406 → 202 LOC.
```

**Atomic Alembic migration `58510d3f6b81`** with matched-state idempotency guard (3-state: fresh / already-migrated / partial-completion abort), 18-column table create + 4 indexes + `INSERT...SELECT` backfill from `probe_run` (`mode='topic_probe'`, scope/commit_sha → `topic_probe_meta` JSON) + drop legacy table. Reversible downgrade with NOT NULL re-COALESCE for `topic`/`intent_hint`/`repo_full_name`. Two prework cycles also landed under v0.4.18: alembic env hardening (`transaction_per_migration=True`) + pre-existing alembic drift cleanup (8 hotpath indexes added to models, `compare_type` callback for SQLite affinity drift, `2d61e9b37427` migration for `uq_prompt_cluster_domain_label` + `global_patterns.id NOT NULL`); lifespan-DDL consolidation into Alembic (`bdd8e96cf489` — 12 startup-time `ALTER TABLE` blocks → single migration; `main.py` 2204 → 1957 LOC).

**ContextVar:** `current_run_id` rebound at canonical `probe_common.py` with `current_probe_id` alias preserving object identity (`current_run_id is current_probe_id` — pinned by `tests/test_probe_service_module_split_v0_4_17.py:27`).

**Frontend:** `SeedModal.svelte` gains additive `runId?: string | null` prop with truthy-guard filter on `seed-batch-progress` events (default `null` preserves pre-P3 global-progress contract; empty-string also treated as no-filter). New `runStatusColor()` utility in `colors.ts` provides chromatic encoding for the 4 RunRow.status values (running cyan / completed green / partial yellow / failed red). New `runs.ts` API client (minimal — backs the inline "Recent Runs" hint inside SeedModal; a dedicated Runs panel is T4 scope).

**Test footprint:** ~94 net new tests across 12 categories per spec § 9 (cat 1 RunRow + migration: 8; cat 2 RunOrchestrator lifecycle: 14; cat 3 RunGenerator protocol: 7; cat 4 TopicProbeGenerator: 14; cat 5 SeedAgentGenerator: 10; cat 6 probes shim: 13; cat 7 seed shim: 8; cat 8 MCP tools: 7; cat 9 /api/runs: 7; cat 10 GC sweep: 4; cat 11 cross-process correlation: 4; cat 12 ProbeRun ORM removal: 6). Backend regression: 3648 passed / 4 skipped (2 pre-existing `test_main.py` lifespan hangs filtered, verified pre-existing on parent commit `2bb8de9a`). Frontend: 1566 passed / 0 failed (97 files). ruff + svelte-check + alembic check all clean.

**Brainstorm decisions (2026-05-06 — all six questions resolved at design time, all six held through implementation):**

| # | Question | Decision (held) |
|---|---|---|
| Q1 | Asymmetry handling | **Asymmetric collapse, one-step.** RunRow + RunOrchestrator + generators shipped in v0.4.18; seed gained row-state for the first time; ProbeRun → RunRow in same migration. No transient SeedRun model. |
...
| Q6 | Rollout | **Two PRs.** PR1 = dark substrate (RunRow + RunOrchestrator + generators + tests, no router wiring). PR2 = wire shims atomically + frontend `run_id` filter additive. Both delivered under PR #70. T4 (v0.4.29) does the unified UI. |
...
**Earlier-draft correction (preserved for archival reference):** the original "ProbeRun has 13 columns, SeedRun has 11 — most overlap" framing in pre-2026-05-06 ROADMAP drafts was incorrect — `ProbeRun` had 17 columns and `SeedRun` never existed. The asymmetry table above captures the verified pre-shipping reality.

---

### Foundation P4 — Long-handler restructures (SHIPPED v0.4.21)
**Status:** **SHIPPED v0.4.21 (2026-05-11)** — 3 TDD cycles (one per handler site) on `feat/foundation-p4`. Integration Test 13 (`test_audit_hook_emits_zero_warn_under_full_pipeline`) PASS — v0.4.22 audit-hook WARN→RAISE flip precondition MET. Detailed shipping notes in [`SHIPPED.md`](SHIPPED.md#v0421--2026-05-11).
...
After Foundation P3 + P4 + Probe T2 + Pattern Graph hardening A–F + Probe T3-T4 ship (target window: v0.4.18-v0.4.29), the architecture is ready for a v0.5.0 major. Candidate themes (none yet committed):
...
**Status:** **Tier 1 SHIPPED** (v0.4.12, 2026-04-29). **Tier 2 SHIPPED** (v0.4.22, 2026-05-16). Tier 3 / Tier 4 remain **Planned**, sequenced AFTER the Pattern Graph architecture hardening 6-sub-project program (v0.4.23–v0.4.27): T3=v0.4.28, T4=v0.4.29. **Substrate unification — originally scoped into T4 — SHIPPED early at v0.4.18 (Foundation P3, 2026-05-09)**, so T3/T4 features now build natively on the unified `RunRow` substrate with zero retroactive migration debt. Tier 2 was bumped repeatedly through public releases — v0.4.13 → v0.4.14 → v0.4.15 → v0.4.20 → v0.4.21 → v0.4.22 — as architectural fixes (SQLite contention, SQLite migration finalization, HistoryPanel pagination, foundation phase, v0.4.19 stability slot, v0.4.20 Pattern Graph canon + auth hotfix slot) shipped first; T3/T4 then deferred a further 6 minor versions for the Pattern Graph architecture hardening program (rationale in the dedicated entry under Immediate). See SHIPPED.md + the "Foundation phase" + "Pattern Graph architecture hardening" entries for complete rationale.
...
**Tier 4 (v0.4.29) — Planned (final UI consolidation):**
...
### Diff-based scene rebuild — post-Sub-project-D follow-on
**Status:** Exploring
**Context:** Pattern Graph hardening Sub-project D extracts the 774-line `rebuildScene` into 5 builder modules (`ClusterBuilder`, `DomainBuilder`, `EdgeBuilder`, `RingBuilder`, `DustBuilder`) sharing a typed `BuilderContext`. The current rebuild flow is full-wipe + rebuild on every data change — every cluster mesh disposed + reconstructed even if only a single cluster's score changed. Once the builder split is in place, the path to a diff-based rebuild becomes tractable: each builder gains a `diff(prev, next, scene, ctx)` method returning add/update/remove sets, and `rebuildScene` becomes a thin orchestrator over diffs.

**Why deferred:** Sub-project D's 32-dispatch scope is already the largest of the program; adding diff-rebuild on top risks scope creep + delays the lifecycle/canon win. Diff-rebuild is also speculative — D's builder boundaries may surface invariants that change the eventual diff API shape. Better to ship the split clean, observe the boundaries in production for a release cycle, then design diff-rebuild against the proven shape.

**Triggers (implement when any becomes a priority):**

1. **Per-frame rebuild cost becomes a hot path** — current full-wipe on every `sceneData` mutation is fine at 50-cluster scale; if topology grows past 200 clusters or rebuild frequency increases (e.g., live scoring updates), diff-rebuild becomes load-bearing.
2. **Animation continuity across rebuilds** — full-wipe interrupts in-flight breathing/pulse animations on unchanged clusters. Diff-rebuild would preserve them.
3. **Sub-project F (Audit-as-Code) catches an invariant** — if F's automated audit detects rebuild cost or animation-discontinuity regressions, diff-rebuild closes the gap.

**Scope estimate:** ~5 builder `diff()` implementations + a top-level diff orchestrator + ~50 new tests (per-builder diff cases) + canon doc updates. Likely 1-2 sessions if D's `BuilderContext` design accommodates the extension cleanly. Smaller if D shipped with the `diff()` hooks as no-op stubs (rejected during Sub-project D brainstorm 2026-05-17 to avoid speculative API surface).

**Spec seed:** would go alongside Sub-project D's spec at `docs/superpowers/specs/2026-05-16-pattern-graph-hardening-program-context.md` when triggered.
...
2. **Replace instant onsets with eased ramps** — pre-fix `mat.emissiveIntensity = baseline * 4` was a synchronous step on the first frame; moving the interpolation into the per-frame tick (cubic ease-out from baseline → peak over 80ms) eliminated the "thud" reading.
...
**Files (when triggered):** depends on which F-feature is being revisited. Canon doc `.claude/skills/brand-guidelines/references/3d-visualization.md` for documentation updates; `frontend/src/lib/components/taxonomy/` for code changes (specifically `BeamShader.ts`, `PlasmaBeam.ts`, `EnvelopePool.ts`, `EnvelopeShader.ts`, `ClusterPhysics.ts`, `TopologyRenderer.ts`, `SemanticTopology.svelte` per-frame callbacks).
...
### `tools/refine.py` handler restructure → **Foundation P4 (v0.4.21)**
...
### `tools/save_result.py` handler restructure → **Foundation P4 (v0.4.21)**
**Status:** SUPERSEDED by Foundation P4 entry above. Retained for cross-reference.
...
### `tools/optimize.py:198` orchestrator restructure → **Foundation P4 (v0.4.21)**
**Status:** SUPERSEDED by Foundation P4 entry above. Retained for cross-reference.
...
**Recent releases:**
- **v0.4.18** (2026-05-09) — Foundation P3 substrate unification: `RunRow` ORM (18 columns, 4 indexes, 4-value status) + `RunOrchestrator` service + `TopicProbeGenerator` + `SeedAgentGenerator` conforming to a `RunGenerator` Protocol; backward-compat preserved across `/api/probes`, `/api/seed`, `synthesis_probe`, `synthesis_seed`; new `GET /api/runs` + `GET /api/seed` + `GET /api/seed/{run_id}` endpoints. `event_bus.subscribe_for_run()` filtered iterator with 500ms ring-buffer replay. `ProbeRun` ORM class + `ProbeService` class deleted; `probe_service.py` 2270 → 37 LOC re-export shim. Plus prework: alembic env hardening + 8 hotpath indexes + `2d61e9b37427` drift-cleanup migration + `bdd8e96cf489` lifespan-DDL consolidation (`main.py` 2204 → 1957 LOC). PR #70, 95+ commits.
