...
| **E** | **Selection State Machine** — wrap the 4–5 parallel selection-related maps (`_flashStates`, `_selectionEngulfed`, `_highlightedId`, `_prevSelectedId`, `focusedNodeId`) into a single `SelectionController` with explicit state-transition diagram. Eliminates the canon F16 highlight-survival hack (the hack is itself a race-condition workaround). | **v0.4.26** | **Planned** (spec seed in program-context doc) | B, C (depends on impact + animation pipelines being stable) | 1–2 sessions. |
...
- **v0.4.26** — Pattern Graph hardening **Sub-project E — Selection State Machine** (consolidate 5 parallel selection maps into one `SelectionController` with explicit state-transition diagram; eliminates canon F16 highlight-survival hack) — **PLANNED**.
