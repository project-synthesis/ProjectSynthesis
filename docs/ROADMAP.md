...
**Snapshot:** v0.4.27 SHIPPED 2026-05-19 — Pattern Graph architecture hardening program **COMPLETE** (all 6 sub-projects A→F shipped across v0.4.23-v0.4.27). Final closure via Sub-project F (Audit-as-Code): `audit-canon.test.ts` (53 tests, one per canon checklist line) + `_canon-coverage.test.ts` meta-test enforce F1-F19 audit gate at CI time. Earlier highlights still in scope: v0.4.22 SHIPPED 2026-05-16 — Probe Tier 2 + audit-hook WARN→RAISE flip + 21 soak findings addressed pre-ship. v0.4.20 SHIPPED 2026-05-10 — Pattern Graph 3D canon F1–F19 retrieval + F19 plasma envelopement burst + GitHub Device Flow auth hotfix chain. v0.4.18 SHIPPED 2026-05-09 — Foundation P3 substrate unification.
...
- **v0.4.28** — Probe Tier 3 (release.sh CI + probe→seed promotion + drill-into-cluster) + **Finding 19b restoration** (was v0.4.23 pre-Pattern-Graph-hardening shift) — **PLANNED**. Finding 19b carryover: wire `bulk_persist` (or per-prompt persist) into `TopicProbeGenerator._run_one_prompt` so the production batch_pipeline path actually saves `Optimization` rows. Pipeline computes scores + embeddings + intent_label + cluster assignment hints today but throws them away — partial regression of the v0.4.12 T1 design intent that ships in v0.4.22 because save-as-suite reads from `RunRow.prompt_results` and works without per-prompt persistence. Estimated scope: ~50 LOC `bulk_persist` call + cluster-assignment timing decisions + partial-batch failure semantics + 5-10 regression tests. See [`docs/SOAK_GATES.md` Day 5 Finding 19b](SOAK_GATES.md) for the deferral rationale.
- **v0.4.29** — Probe Tier 4 (final UI consolidation — substrate already done in P3; was v0.4.24 pre-Pattern-Graph-hardening shift) — **PLANNED**
