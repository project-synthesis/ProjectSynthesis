...
- **v0.4.36** — **Concurrent replay executor** (`PROBE_PROMPT_CONCURRENCY` tier-mapped fan-out + rate-limit projection guard) **+ time-gated debt retirement** (`WriterLockedAsyncSession`, gc shim, 18 unused-ignores, MCP audit hook). — **SHIPPED 2026-06-12** (PR #TBD — back-fill on merge).
