...
**Active program (Pattern Graph architecture hardening):** 6-sub-project refactor of `frontend/src/lib/components/taxonomy/` targeting `SemanticTopology.svelte`'s 2834-line monolith + 228-commit churn since 2026-02-01. Sub-project A (Lifecycle Hardening) spec APPROVED revision 8 + plan APPROVED revision 2 as of 2026-05-16; implementation kickoff pending. Sub-projects B–F Planned with full spec seeds at `docs/superpowers/specs/2026-05-16-pattern-graph-hardening-program-context.md` (resumable from a fresh session). Release allocation: v0.4.23 (A) → v0.4.24 (B) → v0.4.25 (C) → v0.4.26 (D, largest) → v0.4.27 (E) → v0.4.28 (F). Probe Tier 3 shifts to v0.4.29; Tier 4 to v0.4.30. See "Pattern Graph architecture hardening" entry under Immediate for the full 6-sub-project breakdown (A→F, dependency-ordered).
...
**Release allocation:** one version per sub-project. See "Release allocation" table at the top of this file (under "Foundation phase") — Pattern Graph hardening occupies v0.4.23–v0.4.28, pushing Probe Tier 3 to v0.4.29 and Tier 4 to v0.4.30.
...
|---|---|---|---|---|---|
| **A** | **Lifecycle Hardening** — `userData.persistent = true` convention + extracted `cleanupScene` helper; fixes lights/dust/envelope orphan-group class of bugs. Canon F11/F12 (lights, shadows) compliance restored. Replaces ~30 lines of manual save/restore boilerplate with one flag-aware helper call. | **v0.4.23** | **Specs APPROVED 2026-05-16; implementation pending** | nothing | Smallest sub-project (~1 session). Immediate win. New file `scene-cleanup.ts` + tests; constructor edits to `BeamPool` / `EnvelopePool` / `TopologyLabels` / `TopologyRenderer`. Plan: 12 dispatches (10 implementer + 2 reviewer). |
| **B** | **Animation Coordinator** — single per-frame tick with explicit ordered phases (physics → impact → breathing → ambient pulse → camera). Eliminates per-callback registration-order coupling. Phase ordering contract documented in `references/3d-visualization.md`. | **v0.4.24** | **Planned** (spec seed in program-context doc) | A (lifecycle stable before tick pipeline migrates) | 1–2 sessions. ~10 separate `addAnimationCallback` registrations consolidate into one ordered pipeline. |
| **C** | **Impact Coordinator** — single function chains beam → envelope → flash → physics → idle-pulse. Eliminates the 4 duplicate trigger sites (entrance burst, post-growth burst, optimization event, click selection). Canon F7 causal-ordering invariant enforced at source level by routing every site through the same coordinator. | **v0.4.25** | **Planned** (spec seed in program-context doc) | A, B | 1–2 sessions. Today the chain is replicated 4× with subtle differences (e.g., entrance uses `kineticDisplacement=false`, others `true`). |
| **D** | **Scene Builder Extraction** — split the 774-line `rebuildScene` into single-responsibility builder modules: `ClusterBuilder`, `DomainBuilder`, `EdgeBuilder` (hierarchical + similarity + injection), `RingBuilder` (readiness + template), `DustBuilder`. Each unit-testable in isolation. Diff-based rebuild (no full wipe) considered as a follow-on if the builder split makes the diff pattern tractable. | **v0.4.26** | **Planned** (spec seed in program-context doc) | A, B, C | 3–4 sessions. Largest sub-project — 6 cycles × 5 phases + 2 reviewers = 32 dispatches. Target: `SemanticTopology.svelte` < 800 lines (down from 2834), acting as a thin Svelte wrapper around builders + coordinators. |
| **E** | **Selection State Machine** — wrap the 4–5 parallel selection-related maps (`_flashStates`, `_selectionEngulfed`, `_highlightedId`, `_prevSelectedId`, `focusedNodeId`) into a single `SelectionController` with explicit state-transition diagram. Eliminates the canon F16 highlight-survival hack (the hack is itself a race-condition workaround). | **v0.4.27** | **Planned** (spec seed in program-context doc) | B, C (depends on impact + animation pipelines being stable) | 1–2 sessions. |
| **F** | **Audit-as-Code** — automate the `.claude/skills/brand-guidelines/references/3d-visualization.md` F1–F19 audit checklist as a test gate. Every audit checklist line becomes either a source-grep test or a runtime contract test. Failing the audit = failing CI. Replaces today's manual audit-pass. | **v0.4.28** | **Planned** (spec seed in program-context doc) | A, B, C, D, E (every preceding sub-project's contract is one of the things audited) | 1–2 sessions. Final closure of the program. |
...
- **v0.4.23** — Pattern Graph architecture hardening **Sub-project A — Lifecycle Hardening** (`userData.persistent = true` convention + `cleanupScene` helper; fixes 3-light orphan + restores canon F11/F12; closes per-domain `PointsMaterial` leak) — **PLANNED**. Spec APPROVED revision 8 (8 reviewer rounds); plan APPROVED revision 2 (1 reviewer round); 12-dispatch subagent-driven execution per `feedback_tdd_protocol.md` strict 5-phase TDD.
- **v0.4.24** — Pattern Graph hardening **Sub-project B — Animation Coordinator** (single ordered per-frame tick with documented phases: physics → impact → breathing → ambient → camera; eliminates implicit registration-order coupling) — **PLANNED**. Spec seed in program-context doc; brainstorming starts post-v0.4.23 merge.
- **v0.4.25** — Pattern Graph hardening **Sub-project C — Impact Coordinator** (single function chains beam → envelope → flash → physics → idle-pulse; eliminates 4 duplicate trigger sites; F7 causal-ordering enforced at source level) — **PLANNED**.
- **v0.4.26** — Pattern Graph hardening **Sub-project D — Scene Builder Extraction** (split 774-line `rebuildScene` into 5 builder modules: `ClusterBuilder`, `DomainBuilder`, `EdgeBuilder`, `RingBuilder`, `DustBuilder`; target `SemanticTopology.svelte` < 800 lines from 2834) — **PLANNED**. Largest of the 6 sub-projects — ~32 dispatches across 6 cycles.
- **v0.4.27** — Pattern Graph hardening **Sub-project E — Selection State Machine** (consolidate 5 parallel selection maps into one `SelectionController` with explicit state-transition diagram; eliminates canon F16 highlight-survival hack) — **PLANNED**.
- **v0.4.28** — Pattern Graph hardening **Sub-project F — Audit-as-Code** (F1–F19 canon checklist automated as test gate; replaces manual audit-pass) — **PLANNED**. Final closure of the Pattern Graph hardening program.
- **v0.4.29** — Probe Tier 3 (release.sh CI + probe→seed promotion + drill-into-cluster) + **Finding 19b restoration** (was v0.4.23 pre-Pattern-Graph-hardening shift) — **PLANNED**. Finding 19b carryover: wire `bulk_persist` (or per-prompt persist) into `TopicProbeGenerator._run_one_prompt` so the production batch_pipeline path actually saves `Optimization` rows. Pipeline computes scores + embeddings + intent_label + cluster assignment hints today but throws them away — partial regression of the v0.4.12 T1 design intent that ships in v0.4.22 because save-as-suite reads from `RunRow.prompt_results` and works without per-prompt persistence. Estimated scope: ~50 LOC `bulk_persist` call + cluster-assignment timing decisions + partial-batch failure semantics + 5-10 regression tests. See [`docs/SOAK_GATES.md` Day 5 Finding 19b](SOAK_GATES.md) for the deferral rationale.
- **v0.4.30** — Probe Tier 4 (final UI consolidation — substrate already done in P3; was v0.4.24 pre-Pattern-Graph-hardening shift) — **PLANNED**
...
| Q6 | Rollout | **Two PRs.** PR1 = dark substrate (RunRow + RunOrchestrator + generators + tests, no router wiring). PR2 = wire shims atomically + frontend `run_id` filter additive. Both delivered under PR #70. T4 (v0.4.30) does the unified UI. |
...
After Foundation P3 + P4 + Probe T2 + Pattern Graph hardening A–F + Probe T3-T4 ship (target window: v0.4.18-v0.4.30), the architecture is ready for a v0.5.0 major. Candidate themes (none yet committed):
...
**Tier 4 (v0.4.24) — Planned (final UI consolidation):**
