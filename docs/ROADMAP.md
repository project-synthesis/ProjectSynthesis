...
**Active program (Pattern Graph architecture hardening):** 6-sub-project refactor of `frontend/src/lib/components/taxonomy/` targeting `SemanticTopology.svelte`'s 2834-line monolith + 228-commit churn since 2026-02-01. **Sub-projects A (Lifecycle Hardening) + B (Animation Coordinator) SHIPPED 2026-05-17 as combined v0.4.23 release** (PR #83 + PR #84 both completed within a single session window). Sub-projects C–F Planned with full spec seeds at `docs/superpowers/specs/2026-05-16-pattern-graph-hardening-program-context.md` (resumable from a fresh session). Release allocation: v0.4.23 (A + B, SHIPPED) → v0.4.24 (C) → v0.4.25 (D, largest) → v0.4.26 (E) → v0.4.27 (F). Probe Tier 3 shifts to v0.4.28; Tier 4 to v0.4.29. See "Pattern Graph architecture hardening" entry under Immediate for the full 6-sub-project breakdown (A→F, dependency-ordered).
...
| **C** | **Impact Coordinator** — single function chains beam → envelope → flash → physics → idle-pulse. Eliminates the 4 duplicate trigger sites (entrance burst, post-growth burst, optimization event, click selection). Canon F7 causal-ordering invariant enforced at source level by routing every site through the same coordinator. | **v0.4.24** | **Planned** (spec seed in program-context doc) | A, B | 1–2 sessions. Today the chain is replicated 4× with subtle differences (e.g., entrance uses `kineticDisplacement=false`, others `true`). |
...
- **v0.4.24** — Pattern Graph hardening **Sub-project C — Impact Coordinator** (single function chains beam → envelope → flash → physics → idle-pulse; eliminates 4 duplicate trigger sites; F7 causal-ordering enforced at source level) — **PLANNED**.
