...
| **B** | **Animation Coordinator** — single per-frame tick with explicit ordered phases (impact → physics → breathing → ambient pulse → camera). Eliminates per-callback registration-order coupling. Phase ordering contract documented in `references/3d-visualization.md`. | **v0.4.24** | **Planned** (spec seed in program-context doc) | A (lifecycle stable before tick pipeline migrates) | 1–2 sessions. ~10 separate `addAnimationCallback` registrations consolidate into one ordered pipeline. |
...
- **v0.4.24** — Pattern Graph hardening **Sub-project B — Animation Coordinator** (single ordered per-frame tick with documented phases: impact → physics → breathing → ambient → camera; eliminates implicit registration-order coupling) — **PLANNED**. Spec seed in program-context doc; brainstorming starts post-v0.4.23 merge.
