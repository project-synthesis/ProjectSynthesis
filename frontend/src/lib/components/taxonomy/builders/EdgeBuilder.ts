...
function buildMergedCurveGeometry(edges: HierEdge[]): {
  positions: number[];
  indices: number[];
  alphas: number[];
} {
...
function buildSecondaryEdgeGroup(
  data: SceneData,
 ...
 *
 * Each rebuild constructs fresh sub-groups; cleanupScene (Sub-project A)
 * disposes the previous cycle's groups at the start of the next rebuild.
 * No `userData.persistent` flag — edge geometry changes per rebuild
 * because cluster positions change (spec §3.5 EdgeBuilder row).
 *
...
export class EdgeBuilder implements SceneBuilder {
  ...
  build(data: SceneData, scene: THREE.Scene, ctx: BuilderContext): void {
    ...
    for (const edge of data.edges) {
      ...
      // T2.3 — Carry child memberCount so buildMergedCurveGeometry can
    ...
    const simGroup = buildSecondaryEdgeGroup(data, ctx.sceneNodeMap, {
      type: 'similarity',
    ...
    const injGroup = buildSecondaryEdgeGroup(data, ctx.sceneNodeMap, {
      type: 'injection',
