...
// `loadActivityForPeriod` was hosted on `clustersStore` in v0.4.4 and
// moved to `observatoryStore.loadTimelineEvents()` in v0.4.5 to enforce
// separation of concerns between the topology terminal (ActivityPanel)
// and the Observatory timeline. The corresponding tests now live in
// `observatory.svelte.test.ts` (OS8 / OS9 / OS10).

describe('cluster store has no period backfill API (separation-of-concerns guard)', () => {
  beforeEach(() => clustersStore._reset());

  it('does not expose loadActivityForPeriod (moved to observatoryStore)', () => {
    expect(typeof (clustersStore as unknown as Record<string, unknown>).loadActivityForPeriod).toBe('undefined');
  });
});
