...
| `routing.svelte.ts` | Derived routing state mirroring backend 5-tier priority chain. Reactive tier resolver. **Tier-guide trigger consolidation (v0.4.19-dev)**: a single `$effect` in `routes/app/+page.svelte` watches `routing.tier` directly with consumer-side `lastEffectiveTier` `$state` dedup; replaces five fragmented call sites (cold-boot effect, two `routing_state_changed` SSE handler branches, two Settings toggles). `triggerTierGuide()` in `tier-onboarding.svelte.ts` is now a thin pass-through (module-level `lastTriggeredTier` mutable removed — HMR-safe). Critical Svelte 5 invariant: `routing.tier` MUST be read before any early-return gate so the effect always subscribes to tier changes |
| `rate-limit.svelte.ts` | Per-second tick countdown for `rate_limit_active`/`rate_limit_cleared` SSE events. `activeList` entries carry `source` tag (`"heuristic_flags"` / `"sse"` / `"health_poll"`) + `detected_at_ms` for operator-grade detail rows in SettingsPanel rate-limits accordion. `applyHeuristicFlags()` tags client-side detections distinctly from SSE-driven ones |
...
| `sse-health.svelte.ts` | SSE connection health: owns EventSource lifecycle, latency tracking (rolling 100-event window, p50/p95/p99), **4-state degradation detection** (`connecting` / `healthy` / `degraded` / `disconnected` — `connecting` added v0.4.19-dev for the cold-boot transient so StatusBar no longer flashes the red × marker on every fresh page-load before the first `EventSource.open`), exponential backoff reconnection (1s-16s cap, 10 attempts, ±20% jitter), 90s staleness detection. After `MAX_RETRIES=10` exhaustion (~111s) drops into slow-poll cadence (`SLOW_POLL_DELAY_MS=30000`) instead of permanent "Retries exhausted" termination state (v0.4.12). Visibility-change handler retries immediately when the tab regains focus and SSE is disconnected. StatusBar indicator reads `connectionState` and `tooltipText` |
...
src/lib/components/
  ...
  shared/       # CommandPalette, CollapsibleSectionHeader (whole-bar/split modes,
                ...
                # AnimatedDialog (v0.4.19-dev — modal scrim + dialog primitive
                # coordinating transition:fade [scrim] + transition:fly [dialog]
                # via dialogIn/dialogOut presets; ESC + click-outside dismissibility)
  styles/       # shared-keyframes.css (v0.4.19-dev — skeleton-pulse, scrim-in,
                # modal-in, fly-in-bottom, tab-fade-in, phase-reveal-in, status-pulse;
                # imported once via app.css; replaces four legacy duplicates)
...
| `transitions.ts` | `navSlide` / `navFade` + **v0.4.19-dev motion presets**: `tabFade`, `dialogIn`, `dialogOut`, `listInsert`, `listRemove`, `phaseReveal`, `badgeCrossfade`. Powered by an inline 8-iteration Newton-Raphson bezier solver matching `--ease-spring` (`cubic-bezier(0.16, 1, 0.3, 1)`) for entrances + a sibling `easeExit` solver for `cubic-bezier(0.4, 0, 1, 1)` so JS dismissals match CSS exits exactly (Svelte's built-in `cubicOut` drifted visibly). Every Svelte transition site MUST import one of these presets — never hand-roll `{duration, easing}`. Companion: `frontend/src/lib/styles/shared-keyframes.css` hoists `skeleton-pulse`, `scrim-in`, `modal-in`, `fly-in-bottom`, `tab-fade-in`, `phase-reveal-in`, `status-pulse` (imported once via `app.css`) |
| `copy-feedback.svelte.ts` | `useCopyFlash()` Svelte 5 reactive primitive (v0.4.19-dev) — replaces four hand-rolled `setTimeout` timers in Logo / PassthroughView / ForgeArtifact / CodeBlock with one `--duration-copy-flash` window (1500ms in `app.css`). Re-trigger correctly clears prior timers |
| `activity-filters.ts` / `activity-summary.ts` | `isErrorEvent`, `opFamily`, `keyMetric` helpers shared by `ActivityPanel` (topology) + `DomainLifecycleTimeline` (Observatory) — guarantees both surfaces classify ops identically |
