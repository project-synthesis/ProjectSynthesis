...
- **Scoring**: hybrid LLM + heuristic with z-score normalization and divergence detection. **Dimension weights v4 (v0.4.9)**: per-task-type via `get_dimension_weights(task_type)`. Default: faithfulness 0.26, clarity/specificity 0.22, structure/conciseness 0.15. Analysis (audit-class): clarity 0.25 / specificity 0.25 / structure 0.20 / faithfulness 0.20 / conciseness 0.10. `SCORING_FORMULA_VERSION=4`. **F1 backtick specificity** credits backtick-wrapped code identifiers. **F2** raised `ZSCORE_MIN_STDDEV` 0.3→0.5 to bypass z-norm on narrow audit-class distributions. **F4 strategy fidelity**: `OptimizationResult.strategy_used` removed — persisted strategy is always the resolved `effective_strategy`. **F5 `possible_false_premise` flag**: fires when an analysis-class prompt scores LLM faithfulness < 5.0 on a technical-dense prompt — surfaces audits whose surface symbol density masks wrong premises. Purely additive telemetry
