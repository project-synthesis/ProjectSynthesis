...
- **Taxonomy**: evolutionary hierarchical clustering (`services/taxonomy/`). Hot/warm/cold paths. Domain discovery + sub-domain discovery (HDBSCAN within oversized domains). Adaptive merge threshold `0.55 + 0.04 * log2(1 + member_count)` with task_type mismatch penalty (-0.05). Multi-embedding HDBSCAN: blended raw + optimized + transformation vectors (0.65/0.20/0.15) for warm/cold clustering, raw-only for hot-path assignment. Score-weighted centroids. Composite fusion blends 4 signals (topic, transformation, output, pattern) with per-phase adaptive weights. Output coherence informs split/merge lifecycle. Parallel label generation via `asyncio.gather`. Pattern extraction parallelized in Phase 4 (pre-compute taxonomy context sequentially, LLM calls in parallel). **Spectral clustering** replaces HDBSCAN as primary split algorithm (HDBSCAN fallback). Split children start as `state="candidate"`, evaluated by warm-path Phase 0.5 (promote if coherence ≥ 0.30, reject + reassign otherwise). **Cluster dissolution**: small incoherent clusters (coh < 0.30, ≤5 members, ≥2h old) dissolved and members reassigned. Warm-path Phase 0 reconciles domain node member counts and prunes stale archived clusters (>24h, 0 members). `mega_cluster_prevention` gate prevents merge regressions
