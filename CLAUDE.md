...
- `pattern_injection.py` — shared `auto_inject_patterns()` for cluster meta-pattern discovery from taxonomy embedding index (used by internal + sampling pipelines). `InjectedPattern` dataclass carries cluster_label, domain, similarity metadata. `format_injected_patterns()` shared formatter eliminates duplication between pipelines
- `pipeline_constants.py` — shared pipeline constants (`CONFIDENCE_GATE`, `FALLBACK_STRATEGY`, domain discovery thresholds [MIN_MEMBERS=3, MIN_COHERENCE=0.3], `resolve_fallback_strategy()`) for both pipeline implementations
...
- `taxonomy/` — evolutionary taxonomy engine. Process-wide singleton via `get_engine()`/`set_engine()` with thread-safe double-checked locking. `engine.py` orchestrates 3 paths: hot (per-optimization embedding + nearest-node cosine search), warm (periodic HDBSCAN clustering + speculative lifecycle mutations gated by Q_system non-regression + domain discovery + risk monitoring + tree integrity + reconciliation + cleanup), cold (full refit + UMAP 3D projection + OKLab coloring + Haiku labeling). Domain discovery: `_propose_domains()` creates domain nodes from coherent "general" sub-populations with TF-IDF keyword extraction, re-parenting, optimization backfill, and post-discovery re-parenting sweep for stragglers. Same-domain duplicate merge: two-signal detection (label match + centroid >0.40, same-domain embedding >=0.65). Warm-path reconciliation: member_count + coherence recomputed from actual Optimization rows. Zombie cleanup: archives 0-member clusters with stale usage clearing. Stale label/pattern refresh: re-extracts when cluster grows 3x+ since last extraction. Risk detection: `_monitor_general_health()`, `_check_signal_staleness()`, `_refresh_domain_signals()`, `_suggest_domain_archival()`. Tree integrity: `verify_domain_tree_integrity()` (7 checks: orphans, mismatches, persistence, self-refs, duplicates, non-domain parents, archived with usage) + `_repair_tree_violations()` (auto-repair). Stats cache: `get_stats()` cached 30s TTL, invalidated on warm/cold completion. Sub-modules: `clustering.py` (HDBSCAN wrapper), `lifecycle.py` (emerge/merge/split/retire + `GuardrailViolationError` domain assertions + domain inheritance), `quality.py` (5-dimension Q_system with adaptive weights + `coherence_threshold()` domain floor), `projection.py` (UMAP + Procrustes alignment + PCA fallback), `coloring.py` (OKLab from UMAP position + `compute_max_distance_color()` for domains), `labeling.py` (Haiku 2-4 word labels), `snapshot.py` (audit trail CRUD + retention), `sparkline.py` (LTTB downsampling + OLS trend), `family_ops.py` (family CRUD), `matching.py` (cascade search), `embedding_index.py` (numpy index). Key types: `TaxonomyMapping` (domain embed result), `PatternMatch` (cascade search result), `QWeights` (frozen constant-sum weights), `SparklineData`
...
- `clusters.svelte.ts` — cluster state: paste detection (50-char delta, 300ms debounce), suggestion lifecycle (auto-dismiss 10s), cluster tree/stats for topology, cluster detail for Inspector, template spawning, graph invalidation via `taxonomy_changed` SSE. `StateFilter` type with `stateFilter` state, `setStateFilter()` method, and `filteredTaxonomyTree` derived store for graph/navigator filtering
...
src/lib/components/
  ...
  taxonomy/     # SemanticTopology, TopologyControls, TopologyRenderer, TopologyData,
                # TopologyInteraction, TopologyLabels, TopologyWorker (5-force semantic gravity simulation)
...
- **Taxonomy engine**: evolutionary hierarchical clustering in `services/taxonomy/`. Process-wide singleton (`get_engine()`/`set_engine()`) with async lock-gated hot path. Three execution paths: hot (embed + cosine search nearest active node per optimization), warm (periodic HDBSCAN + speculative lifecycle mutations — emerge/merge/split/retire — gated by Q_system non-regression + same-domain duplicate merge + member reconciliation + zombie cleanup + stale label/pattern refresh), cold (full HDBSCAN refit + UMAP 3D projection + OKLab coloring + Haiku labeling). Quality system: 5-dimension Q_system (coherence, separation, coverage, DBCV, stability) with adaptive weights that scale by active node count. Snapshot audit trail records every mutation cycle. Module decomposition: `engine.py` (~500 LOC) + `family_ops.py` (family CRUD) + `matching.py` (cascade search) + `embedding_index.py` (numpy index) extracted from former monolithic `engine.py` (~2100 LOC). Frontend: Three.js `SemanticTopology` with LOD tiers (persistence thresholds: far=0.4, mid=0.2, near=0.0), state-based chromatic encoding (opacity, size multiplier, color override per lifecycle state), raycasting interaction, 5-force semantic gravity simulation (`TopologyWorker`: UMAP anchor, parent-child spring, same-domain affinity, universal repulsion, collision resolution), and `TopologyControls` overlay.
