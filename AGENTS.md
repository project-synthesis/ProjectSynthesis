...
- **Theme**: industrial cyberpunk — dark backgrounds, 1px neon contours, no rounded corners, no shadows. **Applies to 2D Web UI only.** Full 2D-UI brand system in `.claude/skills/brand-guidelines/SKILL.md` (Zero-Effects Directive, Color System, Typography, Animations, Anti-Patterns) + `references/{color-mappings,component-patterns,layout-and-accessibility}.md`. The 3D Pattern Graph / Taxonomy visualization (`frontend/src/lib/components/taxonomy/`) is **exempt** from the 2D-UI rules and is its own medium — fluid, organic, atmospheric, tactile, and cinematic. Canonical visual features (bloom + film grain post-processing, `MeshStandardMaterial` emission, glow textures, `AdditiveBlending` energy cores, Neural Dust galaxy backdrop, organic breathing oscillation, plasma beam tactile feedback, spring physics, smooth catenary edges, etc.) are documented as canon F1–F19 in `.claude/skills/brand-guidelines/references/3d-visualization.md` — that file's "Audit Checklist" is the source-of-truth for what passes 3D brand review.
