...
- **Shares cross-project knowledge** — techniques proven in 5+ clusters across 2+ distinct projects are promoted to durable global patterns and applied everywhere with a 1.3x relevance boost. Single-project patterns stay project-scoped until they generalize
...
- **Agentic targeted exploration** — specify a `topic` (e.g., "embedding cache invalidation in EmbeddingIndex"); the system reads the linked GitHub codebase, generates 5–25 code-grounded prompts citing real identifiers, runs them through the full optimization pipeline, watches the taxonomy emerge new domains/sub-domains organically, and delivers a structured 5-section markdown final report (Top 3 / Score Distribution / Taxonomy Delta / Recommended Follow-ups / Run Metadata) with 4 deterministic recommendation rules. Surfaces: `POST /api/probes` (SSE), `GET /api/probes`, `GET /api/probes/{id}`, `synthesis_probe` MCP tool (15th tool), `scripts/probe.py` CLI shim. 5-phase orchestrator: `grounding → generating → running → observability → reporting`. 7 new `probe_*` taxonomy events + `current_probe_id` ContextVar for cross-event correlation. **All 4 Topic Probe tiers ship within the 0.4.x line:** T1=v0.4.12 (SHIPPED — this), T2=v0.4.21 (Planned — save-as-suite + replay + UI navigator + regression alarm; keys directly off `RunRow.id`), T3=v0.4.22 (Planned — cross-tier composition: probe → seed-agent promotion as a `RunRow.mode` flip + drill-into-cluster + release.sh CI hook), T4=v0.4.23 (Planned — final UI consolidation). **Substrate unification originally scoped for T4 SHIPPED early at v0.4.18 (Foundation P3)** so T2/T3/T4 features now build natively on the unified `RunRow` substrate. Tier 2 was bumped repeatedly through public releases (v0.4.13 → v0.4.14 → v0.4.15 → v0.4.20 → v0.4.21) as architectural fixes (SQLite contention, write-queue finalization, HistoryPanel pagination, foundation phase, v0.4.19 stability slot) shipped first
- **Probe persistence resilience (v0.4.12)** — verify-after-persist gate (no silent success: probe queries DB for canonical truth before reporting; three outcomes — full / partial / catastrophic), per-prompt streaming persistence (smaller transactions + real-time UI updates as each prompt scores via `optimization_created` events), early-abort on catastrophic (cancels in-flight LLM calls within ~75s of first persist trip-wire — saves 12-20 min of Opus 4.7 audit-class calls per failed run). **Architectural fix shipped in v0.4.13**: SQLite writer-slot contention structurally eliminated via the single-writer queue worker (`backend/app/services/write_queue.py`); cycle 10 regression bar (probe + 30 seeds + 100 feedbacks concurrent) goes from 122 `database is locked` lines → 0. **Finalized in v0.4.14**: 100% of short-lived hot+warm writers route through `WriteQueue.submit()`; `WriteQueue.submit_batch()` lands for atomic multi-write groups (OAuth callback + token revoke pair token-write with audit-log in one transaction). See `docs/SHIPPED.md` v0.4.13/v0.4.14 entries for full architecture

### Foundation P3 — Run substrate unification (v0.4.18)
- **Unified `RunRow` model** replaces the legacy probe-only `ProbeRun` and introduces row-state persistence to the seed surface for the first time. 18 columns, 4-value status (`running` / `completed` / `failed` / `partial`), atomic Alembic migration `58510d3f6b81` with idempotency guard + INSERT-SELECT backfill from `probe_run` + drop legacy table; reversible downgrade
- **`RunOrchestrator` service** is the only legitimate writer of `RunRow` rows — owns row creation, status transitions via `WriteQueue.submit()`, cancellation handling under `asyncio.shield()`, exception capture with 2000-char truncation, `current_run_id` ContextVar lifecycle
- **Two pluggable generators conforming to a `RunGenerator` Protocol**: `TopicProbeGenerator` (refactored from the legacy probe service, preserves all 5 phases + 8 SSE event types byte-for-byte) and `SeedAgentGenerator` (refactored from the legacy seed handler, preserves early-failure HTTP 200 + `partial` classification)
- **New unified endpoints** `GET /api/runs` + `GET /api/runs/{run_id}` (paginated, mode/status/project_id filters) + `GET /api/seed` + `GET /api/seed/{run_id}` (additive). `event_bus.subscribe_for_run(run_id)` filtered async iterator with 500ms ring-buffer replay backs race-free SSE on `POST /api/probes` (subscribe-before-dispatch) + the bus→ctx progress bridge for `synthesis_probe`
- **Substrate unification was originally scoped for Topic Probe T4** — shipped early at v0.4.18 so all subsequent Topic Probe tiers (T2=v0.4.21, T3=v0.4.22, T4=v0.4.23) build natively on the unified substrate with zero retroactive migration

### Reliability infrastructure (v0.4.16)
- **Cold-path commit chunking with cumulative Q-gates (P1a)** — replaces whole-refit atomicity with 4-phase SAVEPOINT-bounded execution (`cp_pre_reembed` outer + 4 inner per-phase nested SAVEPOINTs). Q-checks fire after Phase 1 (re-embed) + Phase 2 (reassign); Q regression rolls back to `cp_pre_reembed`, preserving the "Q never drops" invariant. Module-level `_COLD_PATH_LOCK` (asyncio.Lock) serializes concurrent invocations
- **Peer-writer-aware quiescing** — `cluster_metadata["refit_in_progress_until"]` flag (timestamp-expiration is authoritative recovery primitive) lets peer writers (hot/warm/seed/feedback) cooperatively SKIP clusters mid-refit. 4 peer paths integrated with defensive `_parse_quiesce_flag()` for 4 corruption paths
- **8 cold-path decision events** — `lock_acquired`, `cold_path_started/phase_started/phase_committed/q_check/phase_rolled_back/completed`, `silhouette_restored_from_snapshot`. Health endpoint `cold_path` block exposes `last_run_*`, `peer_skip_count_24h`, `rejection_count_24h`, per-phase `p95_phase_duration_ms`
- **Background indexing chunking + WriteQueue migration (P1b)** — `_bg_index` + `RepoIndexService.build_index()` + `incremental_update()` + `invalidate_index()` migrated from legacy session to `WriteQueue.submit()`. Per-batch chunking at `REPO_INDEX_PERSIST_BATCH_SIZE=50`. Per-(repo, branch) `asyncio.Lock` registry serializes concurrent invocations. Lifespan startup sweep `_gc_orphan_repo_index_runs()` recovers stuck `status='indexing'` rows older than 30 minutes. **8 repo-index decision events** + health endpoint `repo_index` block (10 fields)

### Foundation P2 — Probe internals split (v0.4.17)
- **`probe_service.py` shrunk ~12%** — extracted `probe_common.py` (canonical home of `current_probe_id` ContextVar + utility helpers), `probe_phases.py` (Phase 1 grounding helpers), `probe_phase_5.py` (Phase 5 reporting helpers). Public API preserved (`from app.services.probe_service import ProbeService` + `current_probe_id` keep working). Behavior preserved (78 probe tests + 1 backward-compat test passing). Foundation for v0.4.18 substrate unification
...
- **Three-layer codebase context** — cached explore synthesis via Sonnet (architectural overview, once per repo) + per-prompt curated retrieval (full source delivery, 80K char cap, import-graph + doc-ref expansion, source-type balanced, task-gated for non-coding skipping) + performance signals. **Repo relevance gate (B0)** prevents same-stack-different-project contamination via a single cosine floor (0.15) against a project-anchored synthesis — the anchor embedding prepends `Project: {repo_full_name}` and appends up to 500 indexed file paths as `Components:` so focused subsystem prompts latch onto component names, not just the high-level summary. Four-layer indexing cache (curated retrieval invalidation, GitHub tree ETag, content-SHA embedding dedup, file-content TTL) minimizes GitHub API pressure and embedder cost across branches and reindex cycles. Zero request-time LLM calls
...
```bash
# Backend tests (3664 tests)
cd backend && source .venv/bin/activate && pytest --cov=app -v

# Frontend type check (1067 files / 0 errors)
cd frontend && npx svelte-check

# Frontend tests (1753 tests, 80% line coverage baseline)
...
| `/api/seed` | POST | Batch-seed taxonomy. `SeedOutput` gains additive `run_id` (v0.4.18+) |
| `/api/seed` | GET | Paginated `SeedListResponse` filtered to `mode='seed_agent'` rows in the unified `RunRow` substrate (v0.4.18+) |
| `/api/seed/{run_id}` | GET | Full `RunRow` for one batch-seed run (v0.4.18+) |
| `/api/seed/agents` | GET | List available seed agents |
| `/api/runs` | GET | Unified Foundation P3 substrate view — paginated `RunRow` list with `mode?`/`status?`/`project_id?` filters, ordered `started_at desc` (v0.4.18+) |
| `/api/runs/{run_id}` | GET | Full `RunRow` (404 if missing). Single endpoint serving both probe + seed-agent rows (v0.4.18+) |
