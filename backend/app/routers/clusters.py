...
async def get_cluster_activity_history(
    ...
    try:
        ...
        activity_events = [TaxonomyActivityEvent(**e) for e in raw]
        return ActivityHistoryResponse(
            events=activity_events,
            total=offset + len(activity_events) + (1 if has_more else 0),
            has_more=has_more,
        )
...
async def reset_taxonomy(
    ...
    archived_pruned = int(prune_result.rowcount or 0)  # type: ignore[attr-defined]
    await db.commit()
