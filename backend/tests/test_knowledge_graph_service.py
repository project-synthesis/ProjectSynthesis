...
async def test_get_graph_with_data(mock_db):
    f1 = PatternFamily(
        id=str(uuid.uuid4()), usage_count=1, intent_label="create function",
        domain="backend", task_type="coding", avg_score=7.5, member_count=3,
        updated_at=datetime.now(timezone.utc),
    )
    f2 = PatternFamily(
        id=str(uuid.uuid4()), usage_count=1, intent_label="test function",
        domain="backend", task_type="coding", avg_score=8.5, member_count=2,
        updated_at=datetime.now(timezone.utc),
    )
...
async def test_search_patterns(mock_db):
    ...
    p1 = MetaPattern(
        id=str(uuid.uuid4()), pattern_text="use assertions",
        source_count=2, family_id=f1.id,
    )
...
async def test_get_family_detail(mock_db):
    f1 = PatternFamily(
        id=str(uuid.uuid4()), usage_count=1, intent_label="create fun",
        domain="backend", task_type="coding", avg_score=7.5, member_count=3,
        updated_at=datetime.now(timezone.utc),
    )
    def side_effect(query):
