...
The C7 design (per spec sec 4.8 + plan sec Cycle 7), updated for v0.4.34:

1. ``probe_event_correlation`` reads the ``current_run_id`` ContextVar
   declared canonically in ``probe_common.py`` (Foundation P3 Cycle 14
   moved the canonical home; v0.4.34 retired the ``current_probe_id``
   alias). The module also exposes
   ``inject_probe_id(context: dict) -> dict`` (helper name preserved for
   semantic continuity — the injected key remains ``probe_id`` for
...
class TestProbeIdCorrelation:
    """When ``current_run_id`` ContextVar is set, ``log_decision`` should
    inject ``probe_id`` into ``context`` so downstream consumers can filter
    Activity-Panel events by originating probe.

    The injected key remains ``probe_id`` for backward-compat with
    downstream consumers (frontend Activity Panel, ring-buffer queries)
    — only the ContextVar name was retired. The semantic value carried
    is the run id of the currently-executing probe / seed-agent run.
    """
    ...
    def test_probe_id_correlation_when_var_set(
        ...
        token = current_run_id.set("p-corr-set")
        try:
...
class TestCrossProcessForwarding:
    ...
    async def test_cross_process_forward_via_notify_event_bus(
        ...
        token = current_run_id.set("p-cross-1")
        try:
            with patch(
