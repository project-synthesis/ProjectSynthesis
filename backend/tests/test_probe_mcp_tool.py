...
class TestSynthesisProbe:
    ...
    async def test_15th_mcp_tool_registered(self):
        ...
        ``synthesis_save_suite`` + ``synthesis_replay_suite`` shipped) to
        18 (v0.4.29 T3.5, when ``synthesis_refresh_seed_agent`` shipped) to
        19 (v0.4.30 T3.3, when ``synthesis_drill_into_cluster`` shipped).
        ...
        assertion tracks the current 19-tool reality.
        """
